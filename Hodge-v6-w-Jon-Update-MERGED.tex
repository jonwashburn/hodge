...
\begin{enumerate}
\item \textbf{Bergman kernel asymptotics and jet control} (Lemma~\ref{lem:bergman-control}, Remark~\ref{rem:bergman-control-external}): The uniform $C^1$ gradient control on $m^{-1/2}$-balls for holomorphic sections of high tensor powers of ample line bundles.  References: Tian~\cite{Tian90}, Catlin~\cite{Catlin99}, Zelditch~\cite{Zelditch98}, Ma--Marinescu~\cite{MaMarinescu07}.
...
\begin{remark}[External input flag]\label{rem:bergman-control-external}
\textbf{This lemma is an external input.}  It is not proved from first principles in this manuscript.  The statement relies on deep results from complex geometry: Bergman kernel asymptotics on ample line bundles, uniform $C^\ell$ estimates on Bergman-scale balls, and the construction of ``peak sections'' that interpolate prescribed jets.  These are well-established but substantial theorems.  Any adversarial audit of this proof should treat Lemma~\ref{lem:bergman-control} as an imported assumption with the above references as justification.
\end{remark}
...
then
\[
\mathcal F(B_F)\ \le\ \Delta_F\sum_{a=1}^N \Bigl(\Mass(\Sigma(u_a))+\Mass(\partial\Sigma(u_a))\Bigr).
\]
Consequently,
\[
\mathcal F\!\left(\partial T^{\mathrm{raw}}\right)
...
\qquad k:=2n-2p,
\]
and hence, in the common situation where the slice currents on interfaces are cycles (so $\partial\Sigma_F(u_a)=0$), the global estimate
\[
\mathcal F\!\left(\partial T^{\mathrm{raw}}\right)
...
\;+\sum_{a=1}^{N_F} w_a\Bigl(\Mass(\Sigma_{\Phi_{Q',F}y_a})+\Mass(\partial\Sigma_{\Phi_{Q',F}y_a})\Bigr)\Biggr)\ +\ O(\varepsilon\,M_F),
\]
...
\]
\noindent where $\varepsilon_{\mathrm{glue}}=O(\delta+\varepsilon+\mathrm{mesh}+1/m)$.
In the global parameter schedule of \S\ref{sec:parameter-schedule} (choose $m$ first), given any $\tau>0$ we may choose $m$ so large that $1/m<\tau$,
then choose $\delta,\varepsilon$ and the mesh scale so small that $\delta+\varepsilon+\mathrm{mesh}<\tau$, yielding $\mathcal F(\partial T^{\mathrm{raw}})\le C\,\tau\,m=o(m)$ along the mesh refinement.
...
as in Substep~4.2.
Assume $\mathcal F(\partial T^{\mathrm{raw}})$ is as small as desired (e.g.\ in the parameter regime where
$\mathcal F(\partial T^{\mathrm{raw}})\le \varepsilon_{\mathrm{glue}}\,m$ with $\varepsilon_{\mathrm{glue}}\to 0$).
...
\end{proposition}
\begin{proof}
Choose $R,Q$ in the definition of $\mathcal F(\partial T^{\mathrm{raw}})$ with
$\partial T^{\mathrm{raw}}=R+\partial Q$ and $\Mass(R)+\Mass(Q)\le 2\mathcal F(\partial T^{\mathrm{raw}})$.
Fill $R$ by an integral current $Q_R$ with $\partial Q_R=R$ (Federer--Fleming isoperimetric inequality), and set
$R_{\mathrm{glue}}:=-(Q+Q_R)$ as above.
Then $\partial R_{\mathrm{glue}}=-\partial T^{\mathrm{raw}}$ and
$\Mass(R_{\mathrm{glue}})\le \Mass(Q)+\Mass(Q_R)\to 0$ as $\mathcal F(\partial T^{\mathrm{raw}})\to 0$.
\end{proof}
