...
PR 3a: Induced filtrations on graded pieces (Deligne, *Théorie de Hodge II*, §1.2.1).
...
noncomputable def grπ (F : DecFiltration A) (n : ℤ) : (F n : C) ⟶ F.gr n := by
  classical
  -- Unfold `gr` and use the cokernel projection.
  -- The proof term is stable because `Subobject.ofLE` is independent of the proof of `≤`.
  simpa [DecFiltration.gr] using
    (cokernel.π ((F (n + 1)).ofLE (F n) (F.antitone (by omega))))
...
This is Deligne's `F`-filtration on `Gr_G^q(A)` in §1.2.1.
-/
