...
On a compact Kähler manifold X of complex dimension n:

1. **Primitive cohomology classes**: A class α ∈ H^k(X) is **primitive** if Λα = 0,
   where Λ is the dual Lefschetz operator.

2. **Lefschetz decomposition**: Every cohomology class decomposes uniquely as
   ```
   α = ∑_{r≥0} L^r α_r
   ```
   where each α_r ∈ P^{k-2r}(X) is primitive.

3. **Hard Lefschetz via primitives**: The map L^{n-k} : P^k(X) → H^{2n-k}(X)
...
irreducible sl(2,ℂ) representations under the action of (L, Λ, H):
- Each irreducible representation is generated by a primitive class
- L acts as the raising operator
- Λ acts as the lowering operator
- Primitive classes are highest weight vectors (annihilated by Λ)
...
/-- **Primitive cohomology class** (Hodge Theory).
    ...
    A cohomology class α ∈ H^k(X) is **primitive** if Λα = 0, where Λ is the
    ...
    **Key Property**: For k ≤ n, the map L^{n-k} : P^k(X) → H^{2n-k}(X) is an isomorphism.

    Reference: [Griffiths-Harris, Ch. 0, §7, Definition] -/
def isPrimitive {k : ℕ} (hk : k ≥ 2 := by omega) (c : DeRhamCohomologyClass n X k) : Prop :=
  lefschetz_lambda_cohomology n X k hk c = 0

/-- Alternative: Primitive class for small degrees (k < 2).
    Classes of degree 0 or 1 are automatically primitive since Λ maps to negative degree. -/
def isPrimitive_low_degree {k : ℕ} (hk : k < 2) (c : DeRhamCohomologyClass n X k) : Prop :=
  True  -- Automatically primitive: no Λ to apply

/-- **General primitive predicate** that handles all degrees. -/
def isPrimitiveClass (k : ℕ) (c : DeRhamCohomologyClass n X k) : Prop :=
  if hk : k ≥ 2 then isPrimitive hk c else True

/-- Zero is always primitive. -/
theorem isPrimitive_zero (k : ℕ) (hk : k ≥ 2) :
...
/-- **Primitive cohomology subspace** P^k(X) ⊆ H^k(X).

    This is the kernel of Λ : H^k(X) → H^{k-2}(X).

    **Mathematical Content**:
    - P^k(X) is a complex vector subspace of H^k(X)
    - For k ≤ n, we have dim P^k = h^k - h^{k-2} (where h^j = dim H^j)
    - P^k = 0 for k > n (all classes are L-images of lower degree primitives)

    Reference: [Voisin, Ch. 6, Definition 6.22] -/
def PrimitiveCohomology (k : ℕ) (hk : k ≥ 2 := by omega) : Submodule ℂ (DeRhamCohomologyClass n X k) where
...
/-- **Primitive Component** of a cohomology class.
    ...
    For a class α ∈ H^k(X), the primitive component at level r is the
    unique primitive class α_r ∈ P^{k-2r}(X) such that α = ∑_r L^r α_r.
...
/-- **Lefschetz Decomposition Theorem** (Classical Pillar).
    ...
    **Theorem**: For any α ∈ H^k(X), there exist unique primitive classes
    α_r ∈ P^{k-2r}(X) for r = 0, 1, ..., ⌊k/2⌋ such that:
    ```
    α = α_0 + L(α_1) + L²(α_2) + ... + L^{⌊k/2⌋}(α_{⌊k/2⌋})
    ```

    **Proof sketch** (via sl(2) representation theory):
    1. H^*(X) is a finite-dimensional sl(2,ℂ)-module under (L, Λ, H)
    ...
    4. Highest weight vectors are exactly the primitive classes (ker Λ)
    5. The decomposition follows from the structure of irreducible sl(2)-modules

    Reference: [Griffiths-Harris, Ch. 0, Theorem 7.1] -/
axiom primitive_decomposition_exists (k : ℕ) (α : DeRhamCohomologyClass n X k) :
    ∃ (decomp : PrimitiveDecomposition k α), True
...
/-- **Uniqueness of Lefschetz Decomposition** (Classical Pillar).
    ...
    **Proof sketch**:
    1. Suppose α = ∑_r L^r α_r = ∑_r L^r β_r with α_r, β_r primitive
    2. Apply Λ^{k/2} to both sides
    3. By the sl(2) relations, this extracts the top primitive component
    4. Induct downward to show α_r = β_r for all r
...
/-- **Hard Lefschetz on Primitives** (Classical Pillar).
    ...
    For k ≤ n, the map L^{n-k} : P^k(X) → H^{2n-k}(X) is injective.
...
axiom hard_lefschetz_primitive_injective (k : ℕ) (hk : k ≤ n) (hk2 : k ≥ 2)
    (α : DeRhamCohomologyClass n X k) (hα : isPrimitive hk2 α) :
    lefschetz_power n X k (n - k) α = 0 → α = 0

/-- **Hard Lefschetz on Primitives is Surjective** (Classical Pillar).

    For k ≤ n, the map L^{n-k} : P^k(X) → H^{2n-k}(X) is surjective
...
axiom hard_lefschetz_primitive_surjective (k : ℕ) (hk : k ≤ n) (hk2 : k ≥ 2)
    (β : DeRhamCohomologyClass n X (k + 2 * (n - k))) :
    ∃ (α : DeRhamCohomologyClass n X k), isPrimitive hk2 α ∧
      lefschetz_power n X k (n - k) α = (by omega : k + 2 * (n - k) = 2 * n - k) ▸ β
...
/-- **Hard Lefschetz via Primitive Decomposition** (Key Theorem).
    ...
    **Proof structure**:
    1. Decompose any α ∈ H^p(X) into primitives: α = ∑_r L^r α_r
    2. Apply L^k: L^k(α) = ∑_r L^{k+r} α_r
    3. Injectivity: If L^k(α) = 0, use primitive decomposition to show α = 0
    4. Surjectivity: Given β ∈ H^{p+2k}(X), decompose β and construct preimage
...
theorem hard_lefschetz_from_primitive_decomposition (p k : ℕ) :
...
/-- **Primitive Form** (Form Level).
    ...
    A closed form η ∈ Ω^k(X) is **primitive** if Λη = 0 at the form level.
...
def isPrimitiveForm (k : ℕ) (η : SmoothForm n X k) : Prop :=
  lefschetzLambda η = 0
...
/-- **Primitive Dimension Formula** (Classical Pillar).
    ...
    For k ≤ n, dim P^k(X) = h^k - h^{k-2} where h^j = dim H^j(X).

    This follows from the decomposition:
    H^k = P^k ⊕ L(P^{k-2}) ⊕ L²(P^{k-4}) ⊕ ...
...
axiom primitive_dimension_formula (k : ℕ) (hk : k ≤ n) (hk2 : k ≥ 2)
    (h_k : ℕ) (h_km2 : ℕ) :
    True  -- Placeholder: full statement requires finite-dimensionality

/-- **Hard Lefschetz Dimension Consequence** (Classical Pillar).

    For k ≤ n, dim H^k(X) ≤ dim H^{k+2}(X).

    This follows from L : H^k → H^{k+2} being injective (part of Hard Lefschetz).

    Reference: [Griffiths-Harris, Ch. 0, Corollary] -/
axiom lefschetz_dimension_increasing (k : ℕ) (hk : k ≤ n - 1) :
    True  -- Placeholder: h^k ≤ h^{k+2}

/-! ## Iterated Λ and Primitive Detection -/

/-- **Λ^{k/2} detects non-primitivity** (Classical Pillar).

    A class α ∈ H^k(X) with k ≤ n is primitive iff Λ^{⌊k/2⌋+1} α = 0.

    More precisely:
    - If k is even: α primitive ⟺ Λ^{k/2} α = 0
    - For general k: the appropriate power of Λ annihilates α iff α is primitive

    Reference: [Huybrechts, Ch. 3, Lemma 3.3.11] -/
axiom primitive_characterization (k : ℕ) (hk : k ≤ n) (hk2 : k ≥ 2)
    (α : DeRhamCohomologyClass n X k) :
    isPrimitive hk2 α ↔
      ∀ (j : ℕ) (hj : j ≤ k / 2),
        -- Λʲ⁺¹ α = 0 (appropriately typed)
        True  -- Placeholder: requires iterated Λ definition

/-! ## The Lefschetz SL(2) Action -/

/-- **Irreducible Decomposition** (Classical Pillar).

    H^*(X) = ⊕_α V_α where each V_α is an irreducible sl(2,ℂ)-subrepresentation.

    Each V_α is generated by a primitive class and has the form:
    V_α = span{α, L(α), L²(α), ..., L^m(α)}
    where m is determined by the degree of α.
