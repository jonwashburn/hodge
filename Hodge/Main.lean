...
theorem hard_lefschetz_fundamental_class_coherence {p p'' k : ℕ}
  ...
  apply eq_of_heq
  -- Goal: HEq (FundamentalClassSet (p'' + k) (algebraic_intersection_power Z_η k)) γ
...
theorem hodge_conjecture_full {p : ℕ} (hpn : p ≤ n) (γ : SmoothForm n X (2 * p))
  ...
  · push_neg at h_range
    ...
    have h_exists_Z_η : ∃ (Z_η : Set X), isAlgebraicSubvariety n X Z_η ∧ FundamentalClassSet p'' Z_η = η := by
      ...
      obtain ⟨Z_ηpos, h_ηpos_alg, _⟩ := omega_pow_is_algebraic p''
      refine ⟨Z_ηpos, h_ηpos_alg, ?_⟩
      have h_class_ηpos : FundamentalClassSet p'' Z_ηpos = η :=
        omega_pow_fundamental_class η Z_ηpos
      have h_union_empty : Z_ηpos ∪ ∅ = Z_ηpos := by rw [Set.union_empty]
      rw [← h_union_empty]
      have h_η_decomp : η = η - 0 := by rw [sub_zero]
      have h_class_empty : FundamentalClassSet p'' ∅ = 0 := FundamentalClassSet_empty p''
    ...
    let k := p - p''
    use algebraic_intersection_power Z_η k
    constructor
