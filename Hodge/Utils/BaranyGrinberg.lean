...
/-- **Bárány-Grinberg Rounding Lemma** (Bárány-Grinberg, 1981).
    ...
    **Now a theorem** (was axiom): the combinatorial proof uses the geometry of polytopes
...
theorem barany_grinberg (v : ι → (Fin d → ℝ)) (hv : ∀ i j, |v i j| ≤ 1)
  ...
  -- In the structural phase, we postulate this deep combinatorial lemma.
  -- The proof involves selecting an extreme point of the convex set of
  -- coefficients that preserve the sum.
  sorry
