...
/-! ## Hodge Theory Tests (Round 14) -/

-- Test: hodgeDual of zero is zero
example (k : ℕ) : hodgeDual (0 : SmoothForm n X (k + 1)) = 0 :=
  hodgeDual_zero

-- Test: hodgeDual is negation-compatible
example (k : ℕ) (ω : SmoothForm n X (k + 1)) : hodgeDual (-ω) = -hodgeDual ω :=
  hodgeDual_neg ω

-- Test: hodgeDual returns 0 for any input with trivial data
