...
/-- **Axiom: Signed Decomposition exists.**
    Given a rational Hodge class η, there exists a signed decomposition of its representative form.
    This is Lemma 8.7 in the manuscript. -/
axiom signed_decomposition {p : ℕ} (γ : SmoothForm n X (2 * p)) (h_closed : IsFormClosed γ)
    (_h_hodge : isPPForm' n X p γ) (η : DeRhamCohomologyClass n X (2 * p)) (h_rational : isRationalClass η) :
