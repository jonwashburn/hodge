...
public import Hodge.FormalConjecture.Opposed
...
noncomputable def inducedOnGrStep
  ...
  FilteredObject.imageSubobject (C := C) (G.grπ (C := C) (X := X) q)
    ((Subobject.pullback (G.inj (Opposite.op q))).obj (F.step (C := C) (X := X) p))
