...
-- **Note**: `submanifoldIntegral_union` does NOT hold with set-dependent
-- evaluation because `evalPointForSet (Z₁ ∪ Z₂)` may differ from `evalPointForSet Z₁`.
-- This would hold for genuine integration (where all points are sampled), but
-- our current stand-in uses single-point evaluation for tractability.
...
/-- **Pointwise comass at basepoint bounded by global comass** (convenience lemma). -/
...
theorem submanifoldIntegral_abs_le {p : ℕ} (ω : SmoothForm n X (2 * p)) (Z : Set X) :
  ...
  -- Set the evaluation point for readability
