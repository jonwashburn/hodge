...
/-- Helper: cast of zero is zero for SmoothForms. -/
private theorem smoothForm_cast_zero {k k' : ℕ} (h : k = k') :
    h ▸ (0 : SmoothForm n X k) = (0 : SmoothForm n X k') := by
  cases h
  rfl
...
theorem hodgeLaplacian_eq_zero_of_trivial_star {k : ℕ} (hk : 1 ≤ k) (hk' : k + 1 ≤ 2 * n)
  ...
  have hd : @hodgeDual n X _ _ _ _ _ k (smoothExtDeriv ω) = 0 := hodgeDual_eq_zero_of_trivial _
  simp only [hd, add_zero]
  -- omega_dual = h1 ▸ hodgeDual (...) = h1 ▸ 0 = 0 (as cast)
  have hd' : @hodgeDual n X _ _ _ _ _ (k - 1) (h1.symm ▸ ω) = 0 := hodgeDual_eq_zero_of_trivial _
  simp only [hd']
  -- dd_star = h1.symm ▸ smoothExtDeriv (h1 ▸ 0) = h1.symm ▸ smoothExtDeriv 0 = h1.symm ▸ 0 = 0
  have hc1 : h1 ▸ (0 : SmoothForm n X (k - 1)) = (0 : SmoothForm n X ((k - 1) + 1)) :=
    smoothForm_cast_zero h1
  simp only [hc1, smoothExtDeriv_zero]
  have hc2 : h1.symm ▸ (0 : SmoothForm n X ((k-1)+1)) = (0 : SmoothForm n X k) :=
    smoothForm_cast_zero h1.symm
  exact hc2
