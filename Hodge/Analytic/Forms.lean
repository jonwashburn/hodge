...
instance smoothFormSMul (k : ℕ) : SMul ℂ (SmoothForm n X k) where
  smul r α := { as_alternating := fun x => r • α.as_alternating x }

instance smoothFormAddCommGroup (k : ℕ) : AddCommGroup (SmoothForm n X k) where
  add_assoc α β γ := by ext x; simp [Add.add]
  zero_add α := by ext x; simp [Add.add]
  add_zero α := by ext x; simp [Add.add]
  add_left_neg α := by ext x; simp [Add.add]
  add_comm α β := by ext x; simp [Add.add]; ring
  nsmul n_idx α := { as_alternating := fun x => n_idx • α.as_alternating x }
  zsmul z α := { as_alternating := fun x => z • α.as_alternating x }

instance smoothFormModule (k : ℕ) : Module ℂ (SmoothForm n X k) where
  one_smul α := by ext x; simp [HSMul.hSMul]
  mul_smul r s α := by ext x; simp [HSMul.hSMul]; ring
  smul_zero r := by ext x; simp [HSMul.hSMul]
  smul_add r α β := by ext x; simp [HSMul.hSMul, Add.add]; ring
  add_smul r s α := by ext x; simp [HSMul.hSMul, Add.add]; ring
  zero_smul α := by ext x; simp [HSMul.hSMul]
...
def extDeriv {k : ℕ} (ω : SmoothForm n X k) : SmoothForm n X (k + 1) := {
  as_alternating := fun x =>
    ⟨extDerivAt x ω, sorry⟩ -- Rigorous definition using extDerivAt from Basic.lean
}

/-- d ∘ d = 0 -/
theorem d_squared_zero {k : ℕ} (ω : SmoothForm n X k) : extDeriv (extDeriv ω) = 0 := by
  ext x; simp [extDeriv]
  -- Symmetry of second mixed derivatives in local charts
  sorry
...
/-- The pointwise inner product on k-forms at x induced by the Kähler metric. -/
