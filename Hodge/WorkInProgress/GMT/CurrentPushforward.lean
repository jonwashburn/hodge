...
variable {Y : Type u} [MetricSpace Y] [ChartedSpace (EuclideanSpace ℂ (Fin n)) Y]
  ...
  { toFun := T.toFun.comp (smoothFormPullbackLinear (n := n) (k := k) f C hC)
    boundary_bound := by
      ...
      have hcomm :
        ...
        simpa using (smoothExtDeriv_pullback (n := n) (f := f) (ω := ω))
      have hT :
      ...
      have hnorm :
        ...
        simpa using (comass_pullback_le (n := n) (k := k) (f := f) (C := C) hC ω)
      have habs :
          |M| * ‖smoothFormPullback (n := n) (f := f) ω‖ ≤ |M| * (‖ω‖ * C) :=
        mul_le_mul_of_nonneg_left hnorm (abs_nonneg M)
      calc
        |(T.toFun.comp (smoothFormPullbackLinear (n := n) (k := k) f C hC)) (smoothExtDeriv ω)| =
