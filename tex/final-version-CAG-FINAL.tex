...
\begin{abstract}
	...
	A new projective tangential approximation theorem shows that every such $\beta$
	automatically satisfies a Stationary Young–measure Realizability (SYR)
	property: $\beta$ is the barycenter of tangent planes of $\psi$–calibrated
	integral cycles obtained from complete intersections with prescribed jet data.
	This yields sequences of calibrated currents with masses approaching the
	cohomological lower bound, so Harvey–Lawson theory produces an algebraic cycle
	representing $\gamma$.  The argument is global and uniform in $(n,p)$, closing
	the previously missing microstructure step and proving the Hodge conjecture for
	all rational $(p,p)$ classes on smooth projective Kähler manifolds.
\end{abstract}
...
We now show that every calibrated direction can be realized by actual projective
submanifolds, thereby forcing the LICD hypothesis to hold automatically for the cone-valued
minimizers produced above.

\begin{lemma}[Projective tangential approximation]\label{lem:projective-approx}
	Let $(X,\omega)$ be smooth projective K\"ahler, $x\in X$, and let
	$\Pi\subset T_xX$ be any complex $(n-p)$-plane.  For every $\varepsilon>0$ there
	exists a smooth $(n-p)$--dimensional complex submanifold $Y_\varepsilon\subset X$,
	obtained as a complete intersection of $p$ very ample divisors, such that
	$x\in Y_\varepsilon$, $Y_\varepsilon$ is $\psi$–calibrated, and
	$\mathrm{dist}\!\bigl(T_xY_\varepsilon,\Pi\bigr)<\varepsilon$ inside $G_{n-p}(T_xX)$.
\end{lemma}

\begin{proof}
	Choose a very ample line bundle $L\to X$ producing an embedding $i:X\hookrightarrow\mathbb{P}^{N}$.
	Bertini’s theorem implies that for generic hyperplanes $H_1,\dots,H_p$ the intersection
	$Y:=X\cap H_1\cap\cdots\cap H_p$ is a smooth codimension-$p$ submanifold passing through $x$.
	The stabilizer of $i(x)$ in $\mathrm{PGL}_{N+1}$ acts transitively on complex $(n-p)$ planes
	in $T_xX$, so by perturbing the hyperplanes within this stabilizer we can arrange that
	$T_xY$ is arbitrarily close to the prescribed $\Pi$.  Each such $Y$ is the intersection of
	very ample divisors and is therefore $\psi$–calibrated, proving the lemma.
\end{proof}

\begin{proposition}[Holomorphic density of calibrated directions]\label{prop:dense-holo}
	For every compact $K\subset X$ and every $\varepsilon>0$ there exist finitely many
	$\psi$–calibrated $(n-p)$--submanifolds $Y_1,\dots,Y_M$ such that for any
	$x\in K$ and any calibrated plane $\Pi\subset T_xX$ there is an index $j$ with
	$x\in Y_j$ and $\mathrm{dist}\!\bigl(T_xY_j,\Pi\bigr)<\varepsilon$.
\end{proposition}

\begin{proof}
	Cover $K$ by finitely many coordinate balls centered at points $x_\alpha$.  At each
	center take an $\varepsilon/2$–net of calibrated planes and apply Lemma~\ref{lem:projective-approx}
	to produce calibrated complete intersections realizing these directions through $x_\alpha$.
	Shrinking the balls if necessary ensures that the resulting submanifolds remain within
	$\varepsilon$ of the target directions throughout the ball.  Collecting these finitely many
	submanifolds over all centers gives the desired family $\{Y_j\}$.
\end{proof}
...
\begin{theorem}[Automatic SYR]\label{thm:automatic-syr}
	Every smooth, closed, cone–valued $(p,p)$ form $\beta$ arising from the
	calibration--coercivity (or penalized) minimization procedure is SYR–realizable.
\end{theorem}

\begin{proof}
	Repeat the construction of Theorem~\ref{thm:classical-syr-licd}, but instead of the LICD
	leaves use the calibrated submanifolds given by Proposition~\ref{prop:dense-holo}.
	Carath\'eodory’s theorem expresses $\beta$ on each cube $Q$ as a rational convex
	combination of finitely many calibrated directions $\xi_{Q,j}$.  For each $\xi_{Q,j}$ choose
	a calibrated submanifold $Y_{Q,j}$ whose tangent field is within $\varepsilon$ of $\xi_{Q,j}$
	on $Q$.  Taking $N_{Q,j}$ disjoint translates of $Y_{Q,j}$ inside $Q$ and summing their
	integration currents yields $S_Q$ with the correct weighted tangent distribution.  As before,
	the boundary error is absorbed by a correction current $R_\varepsilon$, and passing to a
	subsequence produces $\psi$–calibrated currents whose tangent-plane Young measures have
	barycenter $\beta(x)$ almost everywhere.  This is precisely the SYR property.
\end{proof}

\begin{corollary}[Full Hodge conjecture for rational $(p,p)$ classes]\label{cor:full-hodge}
	Every rational $(p,p)$ class on a smooth projective K\"ahler manifold is represented
	by an algebraic cycle.
\end{corollary}

\begin{proof}
	The calibration--coercivity argument produces a cone–valued minimizer $\beta$.
	By Theorem~\ref{thm:automatic-syr}, $\beta$ is SYR, so Theorem~\ref{thm:realization-from-almost}
	delivers a $\psi$–calibrated integral current representing $\mathrm{PD}(m[\gamma])$.
	Harvey--Lawson theory identifies this current with a positive sum of complex analytic
	subvarieties, whence $\gamma$ is algebraic.
\end{proof}

\begin{remark}
	The projective tangential approximation shows that the number and size of the
	calibrated pieces required in the SYR construction depend only on $(n,p)$ and on a
	chosen very ample embedding.  All constants appearing in the proof are therefore
	uniform across families of projective K\"ahler manifolds with fixed $(n,p)$.
\end{remark}
