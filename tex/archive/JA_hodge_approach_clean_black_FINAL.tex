...
one has $\partial R=0$ automatically, See Federer \cite[4.1.12]{Fed69}.
\end{definition}
...
Any calibrated limit is a positive combination of complex subvarieties \cite{King71,HL82}, hence algebraic on projective manifolds \cite{Chow49,Serre56}.
A signed decomposition reduces the general case to the cone--positive case, and Hard Lefschetz \cite{Voisin02,GH78} treats the full $p$--range.

We retain the term ``calibration--coercivity'' for historical continuity. The explicit calibration--coercivity estimate in Section~\ref{sec:cal-coercivity} is included only for background and is not used in the main proof.
...
See, for example, \cite{Voisin02,Lewis99} for background and survey material on the Hodge conjecture; for foundational perspectives on algebraic cycles, see \cite{Kleiman68,Grothendieck69,Fulton98}.

\subsection*{Route via calibration and energy}

Set the K\"ahler calibration (see Harvey--Lawson \cite{HL82} for calibrated geometry)
\[
...
\begin{remark}
\label{rem:spine-theorem-interpretation}[Calibration--coercivity]\label{thm:cal-coercivity-intro}
the rigorous statement and proof are given in Theorem~\ref{thm:cal-coercivity}.
...
By Theorem~\ref{thm:realization-from-almost}, a subsequential limit is a $\psi$--calibrated integral current; Harvey--Lawson \cite{HL82} then identifies it as a positive sum of complex analytic subvarieties, hence algebraic on projective $X$ by Chow/GAGA \cite{Chow49,Serre56}.
...
complete intersections of hyperplane sections \cite{GH78,Hartshorne77}).
...
By Hard Lefschetz (see, e.g., \cite{Voisin02,GH78,Wells}) (Remark~\ref{rem:lefschetz-reduction}), it suffices to treat the range $p\le n/2$.
...
Almost-calibration implies that any flat/varifold limit of the $T_k$ is $\psi$--calibrated \cite{HL82}.
By Harvey--Lawson \cite{King71,HL82}, the limit is integration along a positive sum of complex analytic subvarieties, hence algebraic on projective $X$ by Chow/GAGA \cite{Chow49,Serre56}.
...
this observation provides optional context and is not required for the main proof.
...
In this section we fix the analytic and geometric setup used throughout (see \cite{GH78,Wells,Voisin02} for standard references on K\"ahler geometry).
...
These conventions will be used systematically in the calibration and current formalisms \cite{Fed69,HL82} and in the quantitative estimates appearing later.
...
\]
If $T$ is a $k$--current on $X$, its \emph{mass} is (see Federer \cite[4.1.7]{Fed69})
\[
...
The Hodge star \cite{GH78,Wells}
\[
...
harmonic representative (Hodge's theorem; see \cite[Ch.~6]{Wells} or \cite[Thm~5.23]{Voisin02}).
...
\alpha^{(p-1,p+1)}.
\]
On a K\"ahler manifold \cite[Ch.~0]{GH78},
\[
...
The Lefschetz decomposition \cite[p.~122]{GH78} expresses any $(p,p)$--form as an orthogonal sum
\[
...
On a K\"ahler manifold one has the commutator identities \cite[p.~111]{GH78}
\[
...
We introduce the calibrated Grassmannian of complex $p$--planes and the pointwise cone geometry determined by the K\"ahler calibration (see Harvey--Lawson \cite{HL82} for the foundational treatment of calibrated geometries).
...
By Carath\'eodory's theorem \cite{Schneider14} for convex cones in finite-dimensional vector spaces, each $\alpha_k$ admits a representation
\[
...
\smallskip\begin{remark}[no cone--to--span distortion needed]
...
\begin{proof}
	...
	(isomorphic to the complex Grassmannian $G_{\C}(p,n)$ \cite{GH78}), hence compact in the
...
$\theta_{1},\ldots,\theta_{p} \in [0,\tfrac{\pi}{2})$ \cite{GH78}, the canonical angles
...
\begin{proof}
	Using the standard principal-angle identity \cite{GH78}
	\(
...
\textbf{Optional background section.} We record standard K\"ahler/Hodge estimates \cite{Wells,Voisin02,GH78} controlling off--type components and the primitive part of a closed form in terms of the energy of a Coulomb potential. These estimates provide context but are not required for the main proof.
...
These estimates provide optional background for the coercivity discussion in Section~\ref{sec:cal-coercivity}.
...
d^{*}d\eta = d^{*}\alpha
\]
admits a unique solution \(\eta\) orthogonal to \(\ker d\) (equivalently, orthogonal to harmonic \((2p-1)\)-forms), giving the Hodge decomposition \cite{Wells,Voisin02}
\[
...
This is a standard elliptic estimate for the Hodge operator $d+d^*$ \cite{Wells,Voisin02} (equivalently for the Laplacian) on the Coulomb slice $d^*\eta=0$, restricted to the orthogonal complement of harmonic forms.
...
We construct an explicit $\varepsilon$--net on the fiberwise calibrated Grassmannian (see \cite{GH78} for the complex Grassmannian) and record covering bounds used later to discretize calibrated directions and control angular errors in quantitative estimates.
...
\smallskip\begin{remark}[choice of the net]
...
By compactness and the standard packing principle \cite{GH78} on compact homogeneous
...
We develop pointwise linear--algebra estimates converting wedge/Pl\"ucker closeness into distance to the calibrated cone, with explicit tracking of constants. These estimates interface with the $\varepsilon$--net from Section~\ref{sec:epsnets} and the calibration--coercivity inequalities in Section~\ref{sec:cal-coercivity}.



\noindent\textbf{Optional background (Hermitian/PSD comparison).}
The material in this block provides context and comparison but is not required for the main proof.
...
These pointwise inequalities are recorded as optional linear-algebra background (nets/Hermitian models) and are not required for the main proof.
...
\begin{lemma}[Hermitian model for $(p,p)$]\label{lem:hermitian-model}
	...
	\begin{enumerate}
		...
		\item the contraction (trace) corresponds to the Lefschetz trace \cite{GH78}:  
...
Denote by $\gharm$ its unique $\omega$--harmonic representative \cite{Wells,Voisin02} and by
...
(equivalently, the closed convex cone generated by the extremal rays associated to complex $(n-p)$--planes; cf.\ Section~\ref{sec:calibrated-grassmannian} and \cite{HL82}).
...
% ------------------------------------------------------------
\subsection*{Alternative approach: a penalized route }
...
K\"ahler (Wirtinger) calibration \cite{HL82} of $\C$--dimension $(n-p)$ planes.  Set $k:=2n-2p$ and write $A=\mathrm{PD}(m[\gamma])\in H_{k}(X,\Z)$ for some $m\ge 1$.
...
\end{itemize}
The exact-class conclusion is enforced by Proposition~\ref{prop:cohomology-match} together with the per-face matching/transport mechanism (Proposition~\ref{prop:integer-transport}). For $p=n/2$, this still requires an additional closure input to guarantee that the filling current $U_\epsilon$ in Proposition~\ref{prop:cohomology-match} can be taken with arbitrarily small mass (Lemma~\ref{lem:borderline-p-half}).
rather than relying on a decay exponent in $h$.
Thus the hypothesis \textnormal{(H2)} in Theorem~\ref{thm:spine-quantitative} holds for $p<n/2$ under the present estimates; the borderline case $p=n/2$ is reduced to supplying one of the additional closure inputs listed in Lemma~\ref{lem:borderline-p-half}.
\end{proposition}
...
(See \cite{Fed69,Sim83} for general compactness results.)
...
By the Federer--Fleming compactness theorem \cite[Thm~4.2.17]{Fed69}:
...
\)
On the other hand, since $\psi$ has comass $\le 1$ (Wirtinger inequality \cite{Fed69,HL82}),
\(
...
By Harvey--Lawson \cite[Thm~4.2]{HL82}: For the K\"ahler/Wirtinger calibration, $\psi$--calibration forces the approximate tangent planes of $T$ to be complex $(n-p)$--planes with the standard complex orientation (Harvey--Lawson
...
By the Lefschetz $(1,1)$ theorem (see, e.g., \cite{Voisin02,GH78,Wells})
...
\label{thm:complete-intersection}
	...
	divisors (see, e.g., \cite{Hartshorne77,Lazarsfeld04I,Lazarsfeld04II,Fulton98}). Then there exists a sequence of integral cycles in the class
...
\begin{proof}[Idea]
	Very ample divisors are represented by smooth hypersurfaces \cite{GH78,Hartshorne77} calibrated by
	$\omega^{n-1}/(n-1)!$. Intersections of $p$ such hypersurfaces produce smooth
	complex submanifolds of codimension $p$ calibrated by $\psi=\omega^{n-p}/(n-p)!$ (Wirtinger calibration \cite{HL82}).
...
\emph{Step 3: Closure by isoperimetric filling.} The sum $\sum_Q S_Q$ has small
	...
	inequality \cite[Thm~6.1]{FF60} on compact Riemannian manifolds and the Federer--Fleming Deformation
	Theorem \cite[4.2.9]{Fed69}, there exists a correction current $R_\varepsilon$ with
...
By Harvey--Lawson
 structure theory \cite{HL82}, a $\psi$-calibrated integral cycle in a K\"ahler manifold is a positive sum of currents of integration
over irreducible complex analytic subvarieties of codimension $p$.
Since $X$ is projective, Chow's theorem \cite{Chow49}
 identifies these analytic cycles with algebraic cycles.
Dividing by $m$ expresses $[\gamma]$ as a rational algebraic cycle.
\end{proof}


\noindent\textbf{Purpose of Steps 1--4.}
The next four steps implement the ``cone-to-cycle'' part of the program: starting from a smooth closed
cone-valued representative $\beta$ of $[\gamma]$, they construct local calibrated laminates and then assemble them
into a global integral current representing $\mathrm{PD}(m[\gamma])$. Step~1 gives a pointwise Carath\'eodory
decomposition of $\beta$ into finitely many extremal calibrated directions. Step~2 replaces these directions by a
finite net with uniform $C^{1}$ control. Step~3 (Theorem~B) builds, on each cube, multi-sheet calibrated pieces whose
$\psi$--mass proportions match those of $\beta$. Step~4 (Theorem~C) patches and quantizes these pieces so the
resulting current is closed and has the correct homology class.
...
By Carath\'eodory's theorem \cite{Schneider14} in $\R^{D}$, $\widehat\beta(x)$ can be written as a convex
...
Carath\'eodory's theorem \cite{Schneider14} in the affine space $H_x\cong \R^{D-1}$ yields a convex representation of $\widehat\beta(x)$
...
Let $P_{\mhol}(z,z')$ be the Bergman kernel \cite{MaMarinescu07}
...
See Ma--Marinescu \cite[Thm~4.2.1]{MaMarinescu07} and \cite[Thm~1.1]{MaMarinescu13OffDiag} for the near-diagonal expansion with $C^2$ control implying \eqref{eq:bergman-weighted-rescaled}.
...
Hence $Y=S^{-1}(0)$ is a smooth complex submanifold of $U$ by the holomorphic implicit function theorem \cite{GH78}.
...
\begin{proposition}[Projective tangential approximation with $C^1$ control]\label{prop:tangent-approx-full}
	...
	\[
	Y=\{s_1=0\}\cap\cdots\cap\{s_p=0\}\subset X,\qquad s_i\in H^0(X,L^{\mhol}),
	\]
	...
	\]
	Here $c>0$ is the constant from Lemma~\ref{lem:bergman-control}, and angles are computed after identifying $T_yX$ with $T_xX$
	by Levi--Civita parallel transport inside $B_{c\,\mhol^{-1/2}}(x)$ (cf.\ \cite{GH78}).
	No global smoothness of $Y$ is asserted: $Y$ may have singularities outside $B_{c\,\mhol^{-1/2}}(x)$.
	Moreover, $Y$ is $\psi$-calibrated on its regular locus (hence its integration current $[Y]$ is $\psi$-calibrated in the sense of Wirtinger).
\end{proposition}

\begin{proof}
	Choose unitary holomorphic coordinates $(w,u)\in\C^p\times\C^{n-p}$ centered at $x$ such that
	$\Pi=\{w=0\}\subset T_x^{1,0}X$. Set $\lambda_i:=dw_i\in T_x^{*(1,0)}X$ for $i=1,\dots,p$, so that $\max_i\|\lambda_i\|=1$.
	Apply Lemma~\ref{lem:bergman-control} with tolerance $\varepsilon/p$ to obtain $\mhol\ge m_{\mathrm{hol},1}(\varepsilon/p)$ and
	sections $s_i\in H^0(X,L^{\mhol})$ with $s_i(x)=0$ and
	\[
	\|ds_i(y)-\lambda_i\|\le \frac{\varepsilon}{p}
	\quad\text{for all } y\in B_{c\,\mhol^{-1/2}}(x).
	\]
	Set $Y:=\bigcap_{i=1}^p\{s_i=0\}$.
	By Lemma~\ref{lem:graph-from-grad} applied to $S=(s_1,\dots,s_p)$ in the $(w,u)$-coordinates (after shrinking $\varepsilon$ if needed to meet its
	smallness threshold), the Jacobian $\partial_w S$ is uniformly invertible on $B_{c\,\mhol^{-1/2}}(x)$; hence $Y$ is a smooth complex submanifold on that ball.
	The complex normal space to $Y$ at $y$ is spanned by $\{ds_1(y),\dots,ds_p(y)\}$, which is $\varepsilon$-close to $\{\lambda_1,\dots,\lambda_p\}$.
	Therefore $T_yY$ is $\varepsilon$-close to $\Pi$ for all $y\in Y\cap B_{c\,\mhol^{-1/2}}(x)$.
	
	Finally, $Y$ is complex analytic, hence $\psi$-calibrated on its regular locus by Wirtinger; see \cite[Thm.~4.2]{HL82}
	(and \cite[Thm.~4.5]{King71} for the current formulation).
\end{proof}

\begin{remark}
	In later applications we use only the transversality of the defining sections on the Bergman ball $B_{c\,\mhol^{-1/2}}(x)$,
	which yields a single-sheet $C^1$ graph there; no global smoothness of $Y$ is needed.
\end{remark}
...
\begin{remark}[Impossibility of pointwise global coverage]
	A stronger ``pointwise coverage'' requirement—namely, that for every $x\in K$ and every calibrated plane
	$\Pi\subset T_xX$ there exists an index $j$ with $x\in Y_j$ and $\dist(T_xY_j,\Pi)<\varepsilon$—is impossible
	when $p\ge 1$ and $K$ contains a nonempty open subset of $X$.
	Indeed, each $(n-p)$-dimensional complex submanifold $Y_j$ has real codimension $2p$ in $X$ and hence empty
	interior in the underlying real manifold; therefore a finite union $\bigcup_j Y_j$ cannot contain an open set.
	Accordingly, we work with the finite-net formulation stated above, which is exactly what the construction provides
	and what is used in the sequel.
\end{remark}
...
The stated $\mathcal{H}^k$ and $\mathcal{H}^{k-1}$ estimates follow from the area formula \cite[3.2.3]{Fed69} applied to $\Gamma$ on $E$ and on rectifiable charts for $F$,
...
\begin{remark}[Geometric meaning of matching across adjacent cells]\label{rem:matching-cells-geometry}
	In the later gluing steps, the translation parameters $t_a$ are encoded in the cell-scale coordinate $w$.
	Since each $Y_a$ is a small-slope graph over the affine plane $P_a$ on $B_{c\mhol^{-1/2}}(x)$,
	the associated footprint $F_a$ (and its intersections with boundary faces) depends Lipschitz-continuously on $t_a$.
	This Lipschitz control is the geometric input used to balance and match boundary masses face-by-face
	when adjoining neighboring cubes.
\end{remark}
...
\]
and, writing $\mathsf{Bar}(S_Q)$ for the (mass--normalized) barycenter of the tangent--plane Young measure of $S_Q$, (Young measures: see \cite{KinderlehrerPedregal91,Pedregal97}).
\[
...
\]
A standard Lipschitz estimate on the Grassmannian \cite{GH78} gives $\|\xi_{\Pi}-\xi_{\Pi'}\|_{\mathrm{HS}}\le C\,\angle(\Pi,\Pi')$,
...
\smallskip\begin{remark}[tracking the conditioning constants]
...
degeneracy loci (vanishing of Pl\"ucker minors \cite{GH78} and coordinate coefficients), hence dense.
...
\smallskip\begin{remark}[per--cube integer matching without a forbidden quantifier swap]
...
(Wasserstein distance; see \cite{Villani03}.)
...
In the global-coherence regime (Proposition~\ref{prop:global-coherence-all-labels}), the boundary slices on $F$ are parameterized by the \emph{same} integer-weighted discrete measure
...
Proposition~\ref{prop:prefix-template-coherence}) and setting
\[
\mu_{Q\to F}\ :=\ (\Phi_{Q,F})_\#\nu^{(N_F)},
\qquad
\mu_{Q'\to F}\ :=\ (\Phi_{Q',F})_\#\nu^{(N_F)}.
...
\emph{rolling ball} principle in convex geometry \cite{Schneider14} (often attributed to Blaschke).
...
(see any standard text on convex bodies, e.g.\ Schneider's \emph{Convex Bodies: The Brunn--Minkowski Theory} \cite{Schneider14}).
\end{remark}
...
For \textnormal{(ii)}, by Proposition~\ref{prop:holomorphic-corner-exit-g1g2} the sheet piece $Y^{Q,a}\cap Q$ is a single
...
in Lemma~\ref{lem:template-edits-oh}.
\end{remark}
...
For example, Lemma~\ref{lem:sphere-quantize-nested} produces a nested ordered sequence on a sphere (uniform density), and scaling embeds it into $B_{C_0\varrho h}(0)$.
...
If the target counts come from rounding a smooth density, Lemma~\ref{lem:slow-variation-rounding} implies $|N_Q-N_{Q'}|/N_Q=O(h)$ in the ``many pieces'' regime.
...
or empirical $W_1$ convergence on compact manifolds \cite{Villani03}.
\end{proof}
...
The following conjecture provides a quantitative target for an alternative ``sliver'' route and is not required for the main proof.
...
See Proposition~\ref{prop:vertex-template-face-edits} for the detailed estimate.
\end{remark}
...
The global activation/gluing bookkeeping (Theorem~\ref{thm:sliver-mass-matching-on-template} and Proposition~\ref{prop:global-coherence-all-labels})
...
one may implement the block-uniform coded interleaving of Definitions~\ref{def:checkerboard-anchoring}--\ref{def:block-uniform-codes}
...
%=========================================================
% Proposition: Global coherence across all direction labels
%=========================================================
\begin{proposition}[Global coherence across all direction labels (B1, packaged)]\label{prop:global-coherence-all-labels}
	...
	Proposition~\ref{prop:holomorphic-corner-exit-L1} (with vertex-star coherence as in the relevant remark) to the template planes provided by
	...
	\begin{enumerate}
		\item[\textnormal{(a)}] (\textbf{Local mass/barycenter accuracy})
		for each cube $Q$ and label $i$ the realized mass in direction $i$ matches $M_{Q,i}$
		up to the rounding error $O(1/N)+O(\varepsilon_h^2)$ from Proposition~\ref{prop:vertex-template-mass-matching};
		
		\item[\textnormal{(b)}] (\textbf{Slow variation})
		for each interior adjacency $Q\sim Q'$ and each shared vertex $v\in Q\cap Q'$, one has
		$|N_{Q,v,i}-N_{Q',v,i}|\lesssim h\min\{N_{Q,v,i},N_{Q',v,i}\}$ on the region where $M_{Q,i}$ is not negligible
		(e.g.\ via Lemma~\ref{lem:slow-variation-rounding} and the $0$--$1$ stability Lemma~\ref{lem:slow-variation-discrepancy});
		
		\item[\textnormal{(c)}] (\textbf{Cohomology periods})
		after clearing denominators by choosing $m$ and applying fixed-dimension discrepancy rounding
		...
		exact integral periods and hence the exact class $\mathrm{PD}(m[\gamma])$ in rational homology
		(Proposition~\ref{prop:cohomology-match});
		
		\item[\textnormal{(d)}] (\textbf{Face mismatch control / global flat-norm boundary})
		for each label $i$ the activation hypotheses \textnormal{(iii)}--\textnormal{(iv)} in Theorem~\ref{thm:sliver-mass-matching-on-template} hold
		(by Corollary~\ref{cor:corner-exit-iii-iv}), and summing the resulting per-label flat-norm mismatch bounds yields
		\[
		\mathcal F(\partial T^{\mathrm{raw}})=o(m)
		\]
		under the parameter regime of Remark~\ref{rem:weighted-scaling}.
	\end{enumerate}
\end{proposition}
...
\begin{proof}
	...
	Proposition~\ref{prop:holomorphic-corner-exit-L1} and the finite-template dictionary from Proposition~\ref{prop:corner-exit-template-net}).
	...
	Using Lemma~\ref{lem:slow-variation-rounding} and Lemma~\ref{lem:slow-variation-discrepancy}, round the real budgets
	...
	(using the coherence guarantee in the relevant remark) to realize the corresponding finite
	...
	(Lemma~\ref{lem:barany-grinberg}) to achieve item \textnormal{(c)}.
	...
	$\ell\le \min\{N_{Q,v,i},N_{Q',v,i}\}$ at each shared vertex $v$, hence the \emph{equal--mass part} matches at zero transport cost.
	Consequently one may take, in Proposition~\ref{prop:transport-flat-glue},
	\[
	\tau_{F,i}=0,
	\qquad
...
%=========================================================
% Proposition: Integral cohomology constraints
%=========================================================
\begin{proposition}[Integral cohomology constraints]\label{prop:cohomology-match}
	...
	\]
	(Here $S_Q:=\sum_{j=1}^N\sum_{a=1}^{N_{Q,j}}[Y_{Q,j}^a]\llcorner Q$ is the local integral current built from the sheet pieces, and
	$S_Q(\Theta_\ell):=\int_{S_Q}\Theta_\ell=\sum_{j,a}\int_{Y_{Q,j}^a\cap Q}\Theta_\ell$.)
...
\begin{proof}
	...
	\]
	which is the quantitative period-matching output of the local Carath\'eodory decomposition of $m\beta$
	...
	\[
	\Mass(U_\epsilon)<\min\Bigl\{\epsilon,\ \frac{1}{4\,M_\Theta}\Bigr\},
	\]
	so that $|\int_{U_\epsilon}\Theta_\ell|<\frac14$ for all $\ell$.
	(A construction of such a $U_\epsilon$ is given in Step~5 below, using Corollary~\ref{cor:global-flat-weighted}.)
	Then $T_\epsilon:=S-U_\epsilon$ is a closed integral cycle.
	
	\smallskip\noindent
	\textbf{Step 5: Construct the tiny-mass boundary correction $U_\epsilon$.}
	Set $R:=\partial S$ and define $\delta:=\mathcal F(R)$ using Definition~\ref{def:flat-norm}.
	...
	\]
	Applying the filling statement used in Corollary~\ref{cor:global-flat-weighted}, obtain an integral $k$--current $R_{\mathrm{glue}}$ with
	\[
	\partial R_{\mathrm{glue}}=-R,\qquad \Mass(R_{\mathrm{glue}})\le \delta + C_X\,\delta^{\frac{k}{k-1}}.
	\]
	Set $U_\epsilon:=-R_{\mathrm{glue}}$. Then $\partial U_\epsilon=R=\partial S$, and $\Mass(U_\epsilon)=\Mass(R_{\mathrm{glue}})$ satisfies the required bound.
	
	\smallskip\noindent
	Since each $\Theta_\ell$ represents an integral cohomology class, the pairing $\int_{T_\epsilon}\Theta_\ell=\langle[\Theta_\ell],[T_\epsilon]\rangle$ lies in $\Z$.
	For each $\ell$ we have
	\[
	\int_{T_\epsilon}\Theta_\ell=\int_{S}\Theta_\ell-\int_{U_\epsilon}\Theta_\ell,
	\]
	so the previous estimates imply $\bigl|\int_{T_\epsilon}\Theta_\ell-mI_\ell\bigr|<\frac12$.
	Because $\int_{T_\epsilon}\Theta_\ell\in\Z$ and $|\int_{T_\epsilon}\Theta_\ell-mI_\ell|<\tfrac12$, we conclude
	$\int_{T_\epsilon}\Theta_\ell=mI_\ell$ for all $\ell$.
	This identifies the Poincar\'e dual class of $T_\epsilon$ with $m\gamma$ (modulo torsion), as claimed.
\end{proof}


%=========================================================
% Proposition: Almost--calibration and global mass convergence
%=========================================================
\begin{proposition}[Almost--calibration and global mass convergence for the glued cycles]\label{prop:almost-calibration}
	...
	\]
	for instance the small-mass gluing corrections produced by the global flat-norm filling step.
...
\begin{proof}
	...
	Finally \textnormal{(iii)} follows from
	\[
	\Mass(T_\epsilon)=\int_{T_\epsilon}\psi+\Def_{\mathrm{cal}}(T_\epsilon)=c_0+\Def_{\mathrm{cal}}(T_\epsilon).
	\]
\end{proof}
...
{We emphasize again: $m$ is fixed in the SYR definition; no $m\to\infty$ limit is taken. Smallness is obtained by taking $\delta,\varepsilon\to0$ and refining the mesh (equivalently choosing $\mhol$ large enough for the local realization scale), as quantified in Lemma~\ref{lem:bergman-control}.}
...
It is tempting to argue that Bergman kernel localization or Tian--Yau--Zelditch universality \cite{Tian90,Zelditch98,Catlin99} alone forces the desired
...
In particular, since $F$ is $C^1$ and $\partial_wF(u,g(u))$ is invertible for all $u\in U_u$, the implicit function theorem \cite{LangGmT} implies $g\in C^1(U_u)$; hence the displayed bound holds for every $u\in U_u$.
...
This step is now achieved by Proposition~\ref{prop:cell-scale-linear-model-graph}, which builds holomorphic complete intersections
...
\item Bergman kernel asymptotics / jet right-inverses \cite{Tian90,Catlin99,Zelditch98,Donaldson01} to achieve the same $C^1$ control directly.
\end{itemize}
\end{remark}
...
$\{\rho/4\le |z|\le \rho/2\}$.
Apply the H\"ormander $L^2$ $\bar\partial$ estimate \cite[Thm~4.2.1]{Hormander65} (see also \cite[Ch.~VIII]{Demailly12}):
...
By the Federer--Fleming
 dual characterization of the flat norm \cite{FF60}, it suffices to test $\partial T^{\mathrm{raw}}$
...
By the Federer--Fleming isoperimetric inequality \cite[Thm~6.1]{FF60} (see also \cite[Thm~4.2.10]{Fed69}):
there exists an integral $k$--current $Q$ in $\R^N$ with $\partial Q=\iota_\# R$ and
\[
\Mass(Q)\ \le\ C_{N,k}\,\Mass(\iota_\# R)^{\frac{k}{k-1}}.
\]
Define $Q_R:=\pi_\# Q$, which is an integral $k$--current in $X$ (pushforward under a Lipschitz map preserves integrality by \cite[Thm~4.1.14]{Fed69}).
Then
\[
...
$H^{2n-2p}(X,\Z)/\mathrm{tors}$ (see \cite{GH78,Voisin02} for Hodge theory on compact K\"ahler manifolds).
...
(See \cite{BaranyGrinberg81,Schrijver86}.)
...
\smallskip\noindent
\textbf{Construction of a small-mass boundary correction (used in Proposition~\ref{prop:cohomology-match}).}
The sum $S:=\sum_Q S_Q$ is supported in the union of cubes and typically has boundary supported on the inter-cube faces.
By the microstructure/gluing estimate in Proposition~\ref{prop:glue-gap}, one has $\mathcal F(\partial S)\to 0$ as the local errors
$\delta,\varepsilon\to 0$ and the mesh size $\to 0$.

Write $k:=2n-2p$ and set $\delta:=\mathcal F(\partial S)$.
...
\[
\Mass(U_\epsilon)\ \le\ \delta + C_X\,\delta^{\frac{k}{k-1}}.
\]
...
Moreover, since $T_\epsilon=S-U_\epsilon$, the mass triangle inequality gives
\[
\Mass(T_\epsilon)\ \le\ \Mass(S)+\Mass(U_\epsilon).
\]
In particular, whenever $\Mass(S)\to m\int_X \beta\wedge\psi$ and $\Mass(U_\epsilon)\to 0$, one has
\[
\limsup_{\epsilon\to 0}\Mass(T_\epsilon)\ \le\ m\int_X \beta\wedge\psi.
\]

\begin{remark}[From the raw mesh current to the hypotheses of Proposition~\ref{prop:almost-calibration}]\label{rem:raw-to-almost-calibration}
	Let $T^{\mathrm{raw}}=T^{\mathrm{raw}}_h$ be the raw mesh current. By Proposition~\ref{prop:glue-gap} there exists a correction
	$R_{\mathrm{glue}}=R_{\mathrm{glue}}(h)$ such that
	\[
	\partial R_{\mathrm{glue}}=-\partial T^{\mathrm{raw}},
	\qquad
	\Mass(R_{\mathrm{glue}})\xrightarrow[h\to 0]{}0.
	\]
	Set $U_h:=-R_{\mathrm{glue}}$ and $T_h:=T^{\mathrm{raw}}-U_h=T^{\mathrm{raw}}+R_{\mathrm{glue}}$.
	Then $\partial T_h=0$ and $\Mass(U_h)\to 0$.
	Moreover, Proposition~\ref{prop:cohomology-match} gives the required period equalities, hence
	\[
	[T_h]=\mathrm{PD}(m[\gamma])\quad\text{in }H_{2(n-p)}(X,\Z)/\mathrm{tors}.
	\]
	Thus the hypotheses of Proposition~\ref{prop:almost-calibration} apply to $T_h$ with $\epsilon=h$.
\end{remark}

\begin{proof}[Proof of Proposition~\ref{prop:almost-calibration}]
	...
	\end{aligned}
	\]
	which proves \textnormal{(ii)}.
	Item \textnormal{(i)} is the de~Rham pairing for the fixed homology class $[T_\epsilon]$ against the closed form $\psi$,
	and \textnormal{(iii)} follows by combining \textnormal{(i)}--\textnormal{(ii)}.
\end{proof}
...
$\psi$-calibrated and realizes the SYR property (see Allard \cite{Allard72} and Simon \cite{Sim83} for varifold compactness).
...
By Federer--Fleming compactness \cite{FF60,Fed69} for integral currents on the compact manifold $X$, using $\sup_k\Mass(T_k)<\infty$ and $\partial T_k=0$, after passing to a subsequence we obtain $T_k\to T$ in the flat norm. \begin{itemize}
...
By the Harvey--Lawson structure theorem \cite[Thm~4.2]{HL82} (see also King \cite[Thm~4.5]{King71}): Since $T$ is an integral $\psi$--calibrated cycle where $\psi$ is the Wirtinger calibration, $T$ is a positive holomorphic $(n-p)$--cycle.
...
Under uniform bounds on $\Mass(T_j)$ and $\Mass(\partial T_j)$, Federer--Fleming compactness \cite[Thm~4.2.17]{Fed69} implies that any flat limit of integral
...
For classical discussion of analytic cycles and related topological obstructions (including torsion phenomena), see \cite{AtiyahHirzebruch62}.
\end{remark}
...
the signed decomposition is finite-dimensional linear algebra inside $H^{2p}(X,\Q)\cap H^{p,p}(X)$ \cite{Voisin02}; it uses only rationality and the existence of a K\"ahler class (after clearing denominators), with no geometric measure input.
\end{remark}
...
This follows from Chow's theorem \cite{Chow49} for projective space together with Serre's GAGA \cite[Thm~1]{Serre56}; see also \cite[App.~B]{GH78} and \cite[Ch.~I, \S1]{Hartshorne77}.
\end{remark}
...
(See \cite{Voisin02,Lewis99} for standard references and surveys.)
...
Chow's theorem \cite{Chow49} identifies analytic cycles with algebraic cycles, and GAGA \cite{Serre56} transfers the resulting cycle class
to the algebraic category (see also \cite{GH78,Hartshorne77}).
...
\bibitem{Wells}
R.~O.~Wells, Jr.\ \emph{Differential Analysis on Complex Manifolds}. Graduate Texts in Mathematics, vol.~65. Springer, New York, 3rd ed., 2008.
\bibitem{BBS08}
