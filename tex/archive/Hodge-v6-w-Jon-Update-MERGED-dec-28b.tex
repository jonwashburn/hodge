...
\begin{lemma}[Interface slice cycles for the gluing estimate (ridge issue isolated)]\label{lem:face-slice-translation-model}
Fix an interior combinatorial interface face $F=Q\cap Q'$ in the mesh used in Substep~4.2, and work in a tubular product chart
identifying a neighborhood of $F$ with $F\times B^{2p}(0,ch)$ in normal coordinates.

\smallskip\noindent
\textbf{(i) Translation.}
In the single-sheet/flat model of Remark~\ref{rem:transport-hypotheses}, each relevant sheet piece is a $C^1$ graph over an affine translate
of a fixed calibrated plane, and the induced slice currents on $F$ depend on the transverse parameter $u\in B^{2p}(0,ch)$ only by translation
in the face chart.

\smallskip\noindent
\textbf{(ii) Ridge boundary and how it is avoided.}
The nontrivial point is to ensure the slice currents used in Proposition~\ref{prop:transport-flat-glue-weighted} are \emph{closed} (no extra boundary supported
on the codimension-$2$ skeleton of $F$).
For this, we do \emph{not} work directly on the sharp polyhedral face patch $F$.
Instead, as part of the mesh geometry (cf.\ the rounded-cell convention in Remark~\ref{rem:smooth-cells}), one replaces each combinatorial interface face
by a \emph{ridge-free interface hypersurface} $F^{\sharp}\subset Q\cup Q'$ obtained by rounding the face near its boundary inside an $O(h^2)$ neighborhood
of the cube skeleton, so that $F^{\sharp}$ is a smooth hypersurface \emph{without boundary} and agrees with $F$ on the $O(h)$ interior scale where the
single-sheet graph control holds.
Then for each sheet piece $[Y]\llcorner Q$ meeting this interface, the induced interface slice current
\[
\Sigma_F(u)\ :=\ \partial\big([Y]\llcorner Q\big)\llcorner F^{\sharp}
\]
is an integral $(2n-2p-1)$--cycle, and in the flat/parallel model it is a translate of a fixed template cycle in the $F^{\sharp}$ chart.

\smallskip\noindent
Consequently, for every interior interface $F$ the mismatch current
\[
B_F:=\bigl(\partial S_Q\bigr)\llcorner F^{\sharp}\ -\ \bigl(\partial S_{Q'}\bigr)\llcorner F^{\sharp}
\]
fits the translation model of Proposition~\ref{prop:transport-flat-glue-weighted} (hence Proposition~\ref{prop:transport-flat-glue} and
Corollary~\ref{cor:global-flat-weighted}) with the required cycle slices.
\end{lemma}
\begin{proof}
The translation dependence in (i) is exactly the discussion in Remark~\ref{rem:transport-hypotheses} (flat template model) together with the $C^1$ graph
stability of the holomorphic realization on a cell scale.

For (ii), the ridge issue arises only because the sharp face patch $F$ has boundary (edges/ridges) in the cubical complex.
The rounded-cell convention (Remark~\ref{rem:smooth-cells}) allows us to choose the interface representative $F^{\sharp}$ so that, inside the same tubular chart,
$F^{\sharp}$ is a smooth hypersurface without boundary that coincides with the flat face away from an $O(h^2)$ neighborhood of the skeleton.
Since each sheet $Y$ has no boundary, the current $\partial([Y]\llcorner Q)$ is supported on $\partial Q$ and has $\partial(\partial([Y]\llcorner Q))=0$.
Restricting to the boundaryless hypersurface $F^{\sharp}$ therefore produces an integral cycle slice on $F^{\sharp}$.
This removes any additional edge-supported boundary term and justifies using Lemma~\ref{lem:flat-translate} in the proof of
Proposition~\ref{prop:transport-flat-glue-weighted}.
\end{proof}
...
as in Substep~4.2.
For every interior combinatorial interface face $F=Q\cap Q'$ (i.e.\ a codimension-$1$ face shared by two distinct cells of the mesh),
let $F^{\sharp}\subset Q\cup Q'$ denote the corresponding ridge-free interface hypersurface from Lemma~\ref{lem:face-slice-translation-model}, and define
the face mismatch current
\[
B_F\ :=\ \bigl(\partial S_Q\bigr)\llcorner F^{\sharp}\ -\ \bigl(\partial S_{Q'}\bigr)\llcorner F^{\sharp}
\]
admits the translation model of Proposition~\ref{prop:transport-flat-glue-weighted} by Lemma~\ref{lem:face-slice-translation-model};
write the resulting parameter multisets as $\{u_{F,a}\}_{a=1}^{N_F}$ and $\{u'_{F,a}\}_{a=1}^{N_F}$.
Assume moreover that there exists a matching
...
Then $U$ is integral, $\partial U=\partial T^{\mathrm{raw}}$, and hence $T$ is an integral cycle.
(Its homology class will be forced to equal $\mathrm{PD}(m[\gamma])$ in Substep~4.3 by a period/rounding argument applied to this \emph{closed} glued cycle.)
Moreover,
\[
...
\]
By the oriented interface decomposition of $\partial T^{\mathrm{raw}}$ on these ridge-free hypersurfaces one has $\partial T^{\mathrm{raw}}=\sum_F B_F$, hence
$\partial U=\partial T^{\mathrm{raw}}$ and $T:=T^{\mathrm{raw}}-U$ is an integral cycle.
...
Fix $\varepsilon>0$, and choose the mesh/activation parameters so that the explicit gluing current $U$ constructed in
Proposition~\ref{prop:glue-gap} satisfies $\Mass(U)\le\varepsilon/2$.
Define the (closed) glued cycle
\[
U_T:=U,
\qquad
T^{(1)}:=T:=T^{\mathrm{raw}}-U_T.
\]
Then $T^{(1)}$ is closed and integral, and $\Mass(U_T)<\varepsilon/2$.
...
\]
Moreover, the explicit gluing correction $U_T$ has arbitrarily small mass (Proposition~\ref{prop:glue-gap}), hence
its pairing with each fixed smooth $\eta_\ell$ is arbitrarily small:
$\bigl|\int_{U_T}\eta_\ell\bigr|\le \|\eta_\ell\|_{C^0}\Mass(U_T)$.
Choosing parameters so that this error is $<\tfrac12$ as well yields
\[
\Bigl|\int_{T^{(1)}}\eta_\ell - m\,I_\ell\Bigr|<1,
\qquad T^{(1)}=T^{\mathrm{raw}}-U_T.
\]
...
Set $R_\varepsilon:=-U_T$ and $T_\varepsilon:=T^{(1)}$.  This satisfies all requirements.
\end{proof}
...
\item \textbf{Federer--Fleming compactness and isoperimetric filling.}
	...
	\item \textbf{Used in this manuscript:} Theorem~\ref{thm:realization-from-almost} (compactness of integral currents under mass bounds); Proposition~\ref{prop:glue-gap} and Substep~4.2 (constructing a small-mass correction $U_T$ with $\partial U_T=\partial T^{\mathrm{raw}}$, either explicitly face-by-face or via the flat-norm/isoperimetric filling).
