...
recorded in \editref{Proposition~\ref{prop:glue-gap}} produces fixed-class integral
...
\begin{definition}[Stationary Young–measure realizability (SYR)]\label{def:syr}
	...
	\smallskip\noindent
	\editref{(The acronym SYR reflects that the construction can be organized via local plane-mixtures / Young-measure bookkeeping.  This extra bookkeeping is \emph{not} needed for the calibrated-limit closure step; only the defect condition above is used in Theorem~\ref{thm:syr}.)}
\end{definition}
...
system $|L^m|$.
\editref{(Parameter convention: in this ``projective/Bergman'' step, $m$ denotes the tensor power of $L$ controlling the Bergman scale $m^{-1/2}$.  This is independent of the homology-multiple used earlier in $A=\mathrm{PD}(m[\gamma])$; when both appear simultaneously, the meaning will be indicated explicitly.)}
...
The construction produces a sequence $T_k := T_{1/k}$ of fixed-class integral cycles with $\Def_{\mathrm{cal}}(T_k)\to 0$.
Applying Theorem~\ref{thm:realization-from-almost} yields a $\psi$-calibrated integral limit current $T$ with $[T]=\mathrm{PD}(m[\gamma])$, hence the algebraicity conclusion by Harvey--Lawson and Chow/GAGA.
\end{proof}
...
\editref{(We avoid the word ``effective'' here, which in algebraic geometry refers to \emph{algebraic} cycles with nonnegative coefficients.)}
\end{definition}
