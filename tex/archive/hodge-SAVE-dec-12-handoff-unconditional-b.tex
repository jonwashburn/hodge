...
Theorem~\ref{thm:local-sheets} produces disjoint $\psi$--calibrated holomorphic pieces $\{Y_{Q,j}^a\}_{j,a}$ with angle control on $Q$ and boundary supported on $\partial Q$.
Consequently, the local sheet sum
\[
S_Q:=\sum_{j,a}[Y_{Q,j}^a]\llcorner Q
\]
is a $\psi$--calibrated integral current on $Q$ (so $\Mass(S_Q)=\langle S_Q,\psi\rangle$), and by choosing the integer counts $N_{Q,j}$ to match the local target
$\psi$--mass budgets $m\int_Q\beta\wedge\psi$ up to $o(1)$ one obtains a global calibrated sheet--sum $S=\sum_Q S_Q$ satisfying the hypothesis \textnormal{(H1)} in
Theorem~\ref{thm:spine-quantitative}.
\end{proposition}
\begin{proof}
This is exactly the output of Theorem~\ref{thm:local-sheets} (items (i)--(iv)), applied cube-by-cube after freezing the local direction data on $Q$,
together with the elementary count selection described in Lemma~\ref{lem:local-bary} / Substep~4.1 of Theorem~\ref{thm:global-cohom}.
Summing over cubes preserves $\psi$--calibration and integrality and yields the global mass-budget estimate.
\end{proof}
...
\[
\mathcal F(\partial T^{\mathrm{raw}}_j)\xrightarrow[j\to\infty]{}0
\]
for the raw sheet sum $T^{\mathrm{raw}}_j=\sum_Q S_{Q,j}$, and hence Proposition~\ref{prop:glue-gap} supplies correction currents $R_{\mathrm{glue},j}$ with
$\partial(T^{\mathrm{raw}}_j+R_{\mathrm{glue},j})=0$ and $\Mass(R_{\mathrm{glue},j})\to 0$.
Moreover, the same discrete choices can be made to satisfy the global period constraints, so the corrected cycles represent $A=\mathrm{PD}(m[\gamma])$ (Proposition~\ref{prop:cohomology-match}).
Thus the hypothesis \textnormal{(H2)} in Theorem~\ref{thm:spine-quantitative} holds in this manuscript.
\end{proposition}
\begin{proof}
In the corner-exit route, Proposition~\ref{prop:global-coherence-all-labels} verifies the activation/slow-variation hypotheses feeding the weighted face-control machinery
(Corollary~\ref{cor:global-flat-weighted}), which yields $\mathcal F(\partial T^{\mathrm{raw}}_j)=o(m)$ and hence $\mathcal F(\partial T^{\mathrm{raw}}_j)\to 0$ since $m$ is fixed.
Proposition~\ref{prop:glue-gap} then produces $R_{\mathrm{glue},j}$ with vanishing mass.
...
to obtain $\mathcal F(\partial T^{\mathrm{raw}}_j)\to 0$.
\end{proof}
...
\begin{proof}
	By Federer–Fleming compactness \cite{FF60,Fed69}, the class and mass bounds yield a subsequence $T_{k_j}\rightharpoonup T$ as integral currents with $[T]=A$ and
	...
	$\psi$–calibrated (in the sense of \cite{HL82}). The Harvey–Lawson structure theorem then implies $T$ is a
...
bound gives tightness; Allard's compactness theorem
...
\begin{proof}
Fix $\beta$ as in the statement and choose global/scale parameters as in \S\ref{sec:parameter-schedule}.
Propositions~\ref{prop:h1-package} and \ref{prop:h2-package} verify hypotheses \textnormal{(H1)}--\textnormal{(H2)} of the spine theorem
(Theorem~\ref{thm:spine-quantitative}), and hence produce a sequence of closed integral cycles $T_j$ with
$[T_j]=\mathrm{PD}(m[\gamma])$ and $\Def_{\mathrm{cal}}(T_j)\to 0$.
Applying Theorem~\ref{thm:realization-from-almost} yields a $\psi$-calibrated integral limit current $T$ with $[T]=\mathrm{PD}(m[\gamma])$.
The algebraicity conclusion follows by Harvey--Lawson and Chow/GAGA (Remark~\ref{rem:chow-gaga}).
\end{proof}
...
This is the standard characterization of strict positivity; for background on positivity cones see Harvey--Lawson~\cite{HL82} or Demailly~\cite{Demailly12}.
...
\begin{itemize}
\item \textbf{Hard Lefschetz theorem (K\"ahler).}
	\begin{itemize}
	\item \textbf{Citation (standard references):} C.~Voisin, \emph{Hodge Theory and Complex Algebraic Geometry, I}, Cambridge Studies in Advanced Mathematics 76, Cambridge University Press (2002), \S5; or D.~Huybrechts, \emph{Complex Geometry: An Introduction}, Springer (2005), \S6.
	\item \textbf{Used in this manuscript:} only to reduce the Hodge conjecture to the range \(p\le n/2\) (Remark~\ref{rem:lefschetz-reduction}).
	\item \textbf{Hypotheses checked here:} \(X\) is assumed smooth compact K\"ahler (indeed projective K\"ahler) throughout the main theorem statements.
	\end{itemize}

\item \textbf{Federer--Fleming compactness / flat norm / isoperimetric filling for integral currents.}
	\begin{itemize}
	\item \textbf{Citation (primary):} H.~Federer and W.~H.~Fleming, \emph{Normal and integral currents}, Annals of Mathematics (2) \textbf{72} (1960), 458--520.
	\item \textbf{Citation (textbooks):} H.~Federer, \emph{Geometric Measure Theory}, Springer (1969); L.~Simon, \emph{Lectures on Geometric Measure Theory}, Proc.\ Centre for Math.\ Analysis, ANU (1983).
	\item \textbf{Used in this manuscript:}
		\begin{itemize}
		\item compactness/subsequence extraction in Theorem~\ref{thm:realization-from-almost};
		\item flat-norm decomposition + filling estimate in the gluing step (Proposition~\ref{prop:glue-gap} and Substep~4.2 / Step~5 in Section~\ref{sec:realization}).
		\end{itemize}
	\item \textbf{Hypotheses checked here:}
		\begin{itemize}
		\item the approximating currents are integral cycles (\(\partial T_k=0\)) with fixed homology class and uniformly bounded mass (Theorem~\ref{thm:realization-from-almost});
		\item the boundary-mismatch currents are supported on a compact manifold with bounded geometry; the filling estimate is applied at the scale where \(\mathcal F(\partial T^{\mathrm{raw}})\to 0\), yielding correction currents with \(\Mass(R_{\mathrm{glue}})\to 0\) (Proposition~\ref{prop:glue-gap}).
		\end{itemize}
	\end{itemize}

\item \textbf{Calibrated currents and the K\"ahler calibration (analyticity/structure).}
	\begin{itemize}
	\item \textbf{Citation (calibrations):} R.~Harvey and H.~B.~Lawson, Jr., \emph{Calibrated geometries}, Acta Mathematica \textbf{148} (1982), 47--157.
	\item \textbf{Citation (positive currents \(\Rightarrow\) analytic cycles, background):} J.-P.~Demailly, \emph{Complex Analytic and Differential Geometry} (online book/notes), chapters on positive closed currents and analytic cycles; see also Y.-T.~Siu, \emph{Analyticity of sets associated to Lelong numbers of positive closed currents}, Inventiones Mathematicae \textbf{27} (1974), 53--156.
	\item \textbf{Used in this manuscript:} the conclusion in Theorem~\ref{thm:realization-from-almost} that a \(\psi\)-calibrated integral current is supported on (and equals a finite positive sum of) complex analytic subvarieties of codimension \(p\).
	\item \textbf{Hypotheses checked here:} the limit current \(T\) is integral, closed, and \(\psi\)-calibrated (Theorem~\ref{thm:realization-from-almost}); in the K\"ahler case the calibrated tangent planes are complex \((n-p)\)-planes a.e., placing \(T\) in the holomorphic/positive \((p,p)\)-current regime where the cited structure theory applies.
	\end{itemize}
