...
$\gamma^+=\gamma+N[\omega^p]$ \emph{effective} (admitting a smooth closed cone-valued representative) for $N\gg1$.
	
For an effective class with cone-valued representative $\beta$, the main construction produces $\psi$--calibrated integral cycles $T_k$
in the fixed class $\mathrm{PD}(m[\gamma^+])$ whose masses converge to the cohomological lower bound
$\Mass(T_k)\to m\int_X \beta\wedge\psi$.
By compactness, a subsequence converges to a $\psi$--calibrated integral current; by Harvey--Lawson this current is integration along a positive sum of complex analytic subvarieties,
...
The main technical ingredient is a \emph{realization/microstructure} theorem: given a smooth closed cone-valued $(p,p)$--form $\beta$ in a rational class, we construct almost-calibrated integral cycles whose masses converge to the cohomological lower bound and whose tangent-plane Young measures have barycenter $\widehat\beta$ (SYR).
...
Finally, a signed decomposition reduces an arbitrary rational Hodge class to the effective (cone-valued) case, and Hard Lefschetz wires the $p$--range cleanly.
...
\begin{editconeblock}
\subsection*{Auxiliary coercivity observation (CPM--bridge regime; not used in the main chain)}
\end{editconeblock}
...
\editcone{E(\alpha) - E(\gamma_{\harm})\ \ge\ \Def_{\cone}(\alpha).}
	\]
\end{theorem}
\editcone{\editref{(See Theorem~\ref{thm:cal-coercivity} in Section~\ref{sec:cal-coercivity} for the proof; this hypothesis is exactly the CPM--bridge assumption that the energy minimizer already lies in the structured cone.)}}
...
\begin{editconeblock}
For \emph{effective} classes $\gamma$---those admitting a smooth closed cone-valued representative $\beta$ with $\beta(x)\in K_p(x)$---the remaining work is the realization/microstructure step:
construct $\psi$--calibrated integral cycles in $\mathrm{PD}(m[\gamma])$ whose masses converge to $m\int_X\beta\wedge\psi$ (SYR).
This is supplied by the corner-exit vertex-template construction and the weighted flat-norm gluing estimates recorded in \editref{Proposition~\ref{prop:glue-gap}} and summarized in Theorem~\ref{thm:automatic-syr}.
The calibrated limit current is a positive sum of complex analytic subvarieties by Harvey--Lawson, hence algebraic on projective $X$ by Chow/GAGA.
\end{editconeblock}
...
\paragraph{1. Reduction to $p\le n/2$ and to effective classes.}
...
reduces the problem to showing that \emph{effective} classes (those admitting smooth closed cone-valued representatives) are algebraic.

\paragraph{2. Realization (SYR) for a cone-valued representative.}
Fix an effective class $\gamma^+$ with a smooth closed cone-valued representative $\beta$.
Section~\ref{sec:realization} constructs, for a fixed integer $m$, a sequence of $\psi$--calibrated integral cycles $T_k$
in the class $\mathrm{PD}(m[\gamma^+])$ such that $\Mass(T_k)\to m\int_X\beta\wedge\psi$
and the associated tangent-plane Young measures have barycenter $\widehat\beta$ (Theorem~\ref{thm:automatic-syr}).
...
Section~\ref{sec:realization} is the heart of the manuscript: it proves the projective tangential approximation and the microstructure/gluing theorem needed to realize smooth cone-valued forms by calibrated holomorphic pieces with vanishing flat-norm boundary, culminating in the SYR summary theorem (Theorem~\ref{thm:automatic-syr}).
Finally, the signed decomposition lemma reduces an arbitrary rational Hodge class to the effective case, and the main theorem follows.
\end{editconeblock}
...
\editref{included for completeness.}
...
and the Kähler angles \editref{yields the following quadratic control estimate.}
...
\begin{theorem}[\editref{Global prefix-template activation / mass matching (template bookkeeping)}]\label{thm:sliver-mass-matching-on-template}
...
\editref{This conjectural sliver route is included only for context; the unconditional proof in this manuscript proceeds instead via the corner-exit vertex-template mechanism
...
\editref{\textbf{Note.} This conjecture is \emph{not used} in the proof of the main theorems; it is stated only as a quantitative target for an alternative ``sliver'' route.}
\smallskip
...
\begin{editrefblock}
\begin{proposition}[Microstructure/gluing estimate (now established)]\label{prop:glue-gap}
\editref{\textbf{Status update (Dec 2025):} The microstructure/gluing estimate needed for the SYR program is supplied by the corner-exit vertex-template
...
By the microstructure/gluing estimate established in \editref{Proposition~\ref{prop:glue-gap}}
...
$(\delta,\varepsilon,\mathrm{mesh},m)$; see \editref{Proposition~\ref{prop:glue-gap}}.
...
\begin{proof}
\editp{Since $p\le n/2$, the $o(m)$ scaling needed in the microstructure/gluing step is available (Remark~\ref{rem:weighted-scaling}).}
This is the content of Steps 1--6 above, using the quantitative microstructure/gluing estimate recorded in \editref{Proposition~\ref{prop:glue-gap}}.
