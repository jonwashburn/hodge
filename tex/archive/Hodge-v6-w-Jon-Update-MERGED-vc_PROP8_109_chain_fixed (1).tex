...
\qquad k:=2n-2p,
\]
and hence, if additionally each interface slice is a cycle (e.g.\ if it is supported away from $\partial F$, so that $\partial\Sigma_F(u_a)=0$), we obtain the simplified global estimate
\[
\mathcal F\!\left(\partial T^{\mathrm{raw}}\right)
