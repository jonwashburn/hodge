...
\begin{remark}[Span and ``orthogonal component'']\label{rem:span-orthogonal}
Write \(\mathcal U:=\mathrm{span}(\Struct)\subset \mathcal X\) for the linear span and \(\mathcal U^\perp\) for its orthogonal complement.
Throughout, \(\mathrm{proj}_{\mathcal U^\perp}\) denotes the orthogonal projection onto \(\mathcal U^\perp\).

In cone applications (e.g.\ Hodge), the role of ``orthogonal component'' is typically played by a \emph{domain-identified} orthogonal summand (off-type plus primitive parts, boundary dispersion, etc.) that controls the distance to the structured cone; the abstract CPM axioms below are written using \(\mathcal U^\perp\) as a placeholder for that identified orthogonal direction.
\end{remark}
...
\[
 d_{\Struct}(x)^2\ \le\ \Cnet\,\min_{\ell,\lambda\in I_\Struct}\norm{x-\lambda\xi_\ell}^2\ \le\ \Cnet\,\Cproj\,\norm{\mathrm{proj}_{\mathcal U^{\perp}}x}^2,
\]
where \(I_\Struct=\mathbb R_{\ge 0}\) in the cone case and \(I_\Struct=\mathbb R\) in the subspace case.
\end{assumption}
...
\[
 \int \norm{\mathrm{proj}_{\mathcal U^{\perp}}\alpha_x}^2\,d\mu(x)\ \le\ \Ceng\,\big(\Energy(\alpha)-\Energy(\alpha_0)\big).
\]
\end{assumption}
...
\begin{proof}
By Assumption~\ref{asmp:projection}, pointwise \(d_{\Struct}(\alpha_x)^2\le \Cnet\,\Cproj\,\norm{\mathrm{proj}_{\mathcal U^{\perp}}\alpha_x}^2\). Integrating and invoking Assumption~\ref{asmp:energy} yields
\[
 \Defect(\alpha)\le \Cnet\,\Cproj\,\int \norm{\mathrm{proj}_{\mathcal U^{\perp}}\alpha_x}^2\le (\Cnet\,\Cproj\,\Ceng)\,(\Energy(\alpha)-\Energy(\alpha_0)).
\]
Rearrange.
\end{proof}
...
\[
  c\ =\ (\Cnet\,\Cproj\,\Ceng)^{-1}.
\]
In the intrinsic cone-projection route (\(\Cnet=1\)), one may take \(\Cproj=2\) (rank-one Hermitian control) and \(\Ceng=1+d\,C_\Lambda^2\) with \(C_\Lambda=d^{-1/2}\), yielding \(c=1/3\) in recorded middle-degree models.

\emph{Realization step (separate input).}
The coercivity statement controls distance to the \emph{strongly positive cone} in \(L^2\), but by itself does not produce an integral holomorphic cycle.
To conclude ``algebraic cycle'' one needs an additional realization theorem upgrading a strongly positive rational class to a \(\psi\)-calibrated integral cycle (cf.\ the classical calibrated-current $\Rightarrow$ analytic-cycle results, plus the missing existence input).
