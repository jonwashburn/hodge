...
\newcommand{\editamirNEW}[1]{\textcolor{red}{#1}}
\newenvironment{editamirblockNEW}{\begingroup\color{red}}{\endgroup}
...
(each a smooth complete intersection in $|L^m|$ for some large $m$) such that:
\begin{enumerate}
...
\end{enumerate}
Moreover, each $Y_{\alpha,j}$ can be chosen so that on the Bergman ball $B_{c\,m^{-1/2}}(x_\alpha)$
...
By the $C^1$ control in Proposition~\ref{prop:tangent-approx-full}, for $m$ large enough the tangent planes of each $Y_{\alpha,j}$
remain $\varepsilon$-close to $T_{x_\alpha}Y_{\alpha,j}$ throughout $B_{c\,m^{-1/2}}(x_\alpha)$.
...
\editamir{For $Q$ small enough (equivalently, for mesh $h$ small enough), there is a holomorphic chart}
...
For fixed $j$, the $\psi$--mass of a flat model translate in $Q$ is
\[
...
\begin{proof}
\editamir{For the first claim, since $(P+t_a)\cap Q\neq\emptyset$ and $\mathrm{diam}(Q)=h$, after translating the transverse coordinates we may assume the set
$\{t_a\}$ lies in a transverse ball $B_{C h}(0)\subset P^\perp$ for a dimensional constant $C$.}
\editamir{The balls $B(t_a,5\varepsilon h)\subset P^\perp$ are pairwise disjoint and contained in $B_{(C+5\varepsilon)h}(0)$.}
