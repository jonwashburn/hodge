...
% ==========================================================
% Edit highlighting (disabled for journal-ready output)
% ==========================================================
\newcommand{\editblue}[1]{#1}
\newenvironment{editblock}{}{}
\newcommand{\editref}[1]{#1}
\newenvironment{editrefblock}{}{}
\newcommand{\editamir}[1]{#1}
\newenvironment{editamirblock}{}{}
\newcommand{\editp}[1]{#1}
\newenvironment{editpblock}{}{}
\newcommand{\editcone}[1]{#1}
\newenvironment{editconeblock}{}{}
...
as in Substep~4.2.
Assume that for every interior interface $F=Q\cap Q'$ the face mismatch current
\[
B_F\ :=\ \bigl(\partial S_Q\bigr)\llcorner F\ -\ \bigl(\partial S_{Q'}\bigr)\llcorner F
\]
...
$U_h$ with $\partial U_h=\partial T^{\mathrm{raw}}$ and $\Mass(U_h)=o(m)$; consequently
\[
\mathcal F\!\left(\partial T^{\mathrm{raw}}\right)\ \le\ \Mass(U_h)\ =\ o(m).
\]
\end{proposition}
\begin{proof}
By assumption, for each boundary face $F\subset\partial B$ we have an integral $(p,p)$-current
$Q_F$ supported in $U_F$ with $\partial Q_F=B_F$. Define
\[
U_\varepsilon:=\sum_{F\subset\partial B} Q_F.
\]
Since $\partial$ is linear, $\partial U_\varepsilon=\sum_F \partial Q_F=\sum_F B_F$.
By the construction of the oriented face-decomposition $\partial T^{\rm raw}=\sum_F B_F$,
hence $\partial U_\varepsilon=\partial T^{\rm raw}$ as claimed.

Finally, the mass bound follows from the triangle inequality and the hypothesis:
\[
\Mass(U_\varepsilon)\le \sum_F \Mass(Q_F)\le C\sum_F \Delta_F.
\]
\end{proof}
