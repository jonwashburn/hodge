...
\begin{theorem}[Calibration--Coercivity]\label{thm:cal-coercivity-intro}
	...
	\]
\end{theorem}
\editref{(See Theorem~\ref{thm:cal-coercivity} in Section~\ref{sec:cal-coercivity} for the explicit proof and the constant computation.)}
...
recorded in \editref{Proposition~\ref{prop:glue-gap}}.  The projective tangential
...
estimate used later in \editref{Section~\ref{sec:cal-coercivity}}.
...
with $\lambda_{\min}(W(x)) \ge c_0 > 0$.  Choose \editref{$N\in\Q_{>0}$ with $N > M/c_0$
(possible since $\Q$ is dense in $\R$).}
Then for all $x \in X$:
\[
