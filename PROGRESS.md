...
| 2026-03-16 | cursor-session | Prove smoothness of the Euclidean model-current regularizer and package `EuclideanCurrentRegularizationData` | Eliminated the last infrastructure `sorry` in `Hodge/WorkInProgress/GMT/EuclideanCurrentRegularization.lean`; full build, dependency check, and stub audit pass |
| 2026-03-16 | cursor-session | Replace the WIP `CurrentRegularizationLemmas` stub with an explicit blocker note | Confirmed a modeling/interface gap: `CurrentRegularizationData` lacks boundary-compatibility and zero-preservation laws, so the honest next step is to strengthen that interface before instantiating PD regularization lemmas |
| 2026-03-16 | cursor-session | Add `Current.isCycleAt` and `CurrentRegularizationLaws`, then derive `CurrentRegularizationLemmas` generically | The regularization-law interface is now in source, `IntegrationCurrent.lean` proves the cycle/empty bridge lemmas, and `CurrentRegularizationImpl.lean` now routes through the new law package; full build, dependency check, and stub audit still pass |
| 2026-03-16 | cursor-session | Replace `mollifyWeighted` with a chartwise global regularizer | The manifold gluing obstruction is gone: `mollifyWeighted` is now defined pointwise from `mollifyChart ε x T`, proved smooth by comparison with the old partition construction, and proved closed on cycles locally from a fixed chart mollifier; the remaining honest blockers are the concrete Euclidean closedness law and the unresolved global `ChartSmoothData`/`ChartDerivBoundData` scaffolding |
| 2026-03-17 | cursor-session | Eliminate deep regularization placeholders via WIP mollifier pipeline | The 3 opaque axioms in `CurrentRegularizationImpl.lean` are gone; replaced by `euclidean_regularize_isClosed_of_isCycleAt` (Euclidean convolution closedness), `chart_deriv_bound_exists` (chart derivative bounds on compact manifolds), and `chart_contMDiff` (global chart smoothness extension); `PoincareDualityFromCurrentsData` now routes through the honest mollifier path |
