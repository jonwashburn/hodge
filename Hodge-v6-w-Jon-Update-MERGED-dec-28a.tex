...
\begin{lemma}[Face-slice translation/cycle model from the sheet construction]\label{lem:face-slice-translation-model}
Fix an interior interface $F=Q\cap Q'$ in the mesh used in Substep~4.2, and work in a tubular product chart
identifying a neighborhood of $F$ with $F\times B^{2p}(0,ch)$ in normal coordinates.
For each direction label and each sheet piece $[Y]\llcorner Q$ meeting $F$, the induced \emph{face slice} current
\[
\Sigma_F(u)\ :=\ \partial\big([Y]\llcorner Q\big)\llcorner F
\]
is an integral $(2n-2p-1)$--\emph{cycle} on $F$ (so $\partial\Sigma_F(u)=0$), and in the flat/parallel model it depends on the transverse parameter
$u\in B^{2p}(0,ch)$ only by translation in the face chart: $\Sigma_F(u)=(\tau_u)_\#\Sigma_F(0)$.
...
In particular, the slice currents used in that model are genuinely closed (so there is no “extra boundary along edges’’ in the sharp-cube picture).
\end{lemma}
\begin{proof}
In the flat template model of Remark~\ref{rem:transport-hypotheses}, each sheet is an affine calibrated plane section
$(\widetilde\Pi+t)\cap Q$ inside a product chart, and the face slice current $\Sigma_F(u)$ is obtained by translating the parameter
$u$ in the transverse factor; this gives $\Sigma_F(u)=(\tau_u)_\#\Sigma_F(0)$.

The only remaining point is that $\Sigma_F(u)$ is \emph{closed} as a current on $F$.
For this we use the smooth-interface version of Substep~4.2 (already invoked in Corollary~\ref{cor:global-flat-weighted} and
Lemma~\ref{lem:uniformly-convex-slice-boundary}): we work with $C^2$ uniformly convex cells of comparable diameter $h$
(rounded cubes/balls) whose \emph{pairwise interfaces} $F=Q\cap Q'$ are smooth hypersurfaces \emph{without boundary} inside the chosen tubular chart.
Then for any smooth sheet $Y$ with no boundary one has
\(
\partial([Y]\llcorner Q)=[Y]\llcorner \partial Q
\)
and $[Y]\llcorner \partial Q$ is an integral cycle (intersection of manifolds without boundary).
Restricting to the interface hypersurface $F$ inside the chart therefore produces an integral cycle on $F$.
\end{proof}
...
as in Substep~4.2.
For every interior interface $F=Q\cap Q'$ (i.e.\ a codimension-$1$ face shared by two distinct cells of the mesh),
