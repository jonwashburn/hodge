...
However, the “sliver’’ mechanism (Remark~\ref{rem:sliver} / Conjecture~\ref{conj:sliver-local}) benefits from working in cells with \emph{smooth convex}
...
\begin{theorem}[Global cohomology quantization (conditional on flat-norm gluing)]\label{thm:global-cohom}
...
\smallskip
\noindent\textbf{Conditionality.} The existence of the small-mass correction
$R_\varepsilon$ is exactly the \emph{microstructure/gluing} step; see
Hypothesis~\ref{hyp:MG} and Remark~\ref{rem:glue-gap}.  Everything else in the theorem is obtained by
local sheet construction plus fixed-dimension rounding.
\end{theorem}
...
flat/graph models used here.  Establishing such a bound is the remaining analytic bookkeeping needed to make the “sliver’’ escape compatible with the
transport/flat-norm gluing route.
\end{remark}
...
Assuming the hypotheses of Proposition~\ref{prop:template-flat-glue}, one has
\[
\mathcal F(\partial T^{\mathrm{raw}})\ \le\ C_3(h+\varepsilon)\,\Mass(T^{\mathrm{raw}})
\ \asymp\ C_3(h+\varepsilon)\,m.
\]

\smallskip\noindent
\textbf{Fixed $m$: absolute smallness.}
To obtain a gluing correction of arbitrarily small \emph{absolute} mass at a fixed
denominator-clearing multiplier $m$, it suffices to choose $h$ and $\varepsilon$ so that
$C_3(h+\varepsilon)m\le \eta$ for a target $\eta>0$.
Then $\mathcal F(\partial T^{\mathrm{raw}})\le \eta$, and
Proposition~\ref{prop:MG-filling} produces an integral correction current
$R_{\mathrm{glue}}$ with $\partial R_{\mathrm{glue}}=-\partial T^{\mathrm{raw}}$ and
\[
\Mass(R_{\mathrm{glue}})\ \le\ 2\eta + C\,\eta^{\frac{k}{k-1}}
\xrightarrow[\eta\to 0]{}0,
\qquad k:=2n-2p.
\]

\smallskip\noindent
\textbf{$m$--dependent rate: when isoperimetric filling is $o(m)$.}
If one wants an explicit $m$--dependent schedule, take for any $\alpha>0$,
\[
h:=m^{-\alpha},\qquad \varepsilon:=m^{-\alpha},
\]
which yields $\mathcal F(\partial T^{\mathrm{raw}})\lesssim m^{1-\alpha}$.
By Proposition~\ref{prop:MG-filling}, the resulting filling has
$\Mass(R_{\mathrm{glue}})=o(m)$ provided $\alpha>\tfrac{1}{k}$
(equivalently $\mathcal F(\partial T^{\mathrm{raw}})=o(m^{(k-1)/k})$).
\smallskip
...
In higher codimension and fixed $m$, one must also address the “degrees of freedom’’ tension of Remark~\ref{rem:param-tension}
(e.g.\ via the sliver regime of Remark~\ref{rem:sliver} / Conjecture~\ref{conj:sliver-local}).
\end{remark}
...
\item use a true “sliver’’ mechanism (Conjecture~\ref{conj:sliver-local}) that splits the target cube mass into many much smaller local pieces,
...
\begin{corollary}[Holomorphic upgrade on a ball cell (conditional on Bergman control)]\label{cor:holomorphic-flat-sliver-local}
...
Proposition~\ref{prop:flat-sliver-local} shows that the \emph{transverse-measure approximation} requirement in
Conjecture~\ref{conj:sliver-local} is achievable in a clean flat ball model using exact affine calibrated pieces.
...
\begin{remark}[Why Conjecture~\ref{conj:sliver-local} would close the fixed-$m$ tension]\label{rem:sliver-close}
If Conjecture~\ref{conj:sliver-local} holds uniformly on each cube (with $M\sim m\int_Q\beta\wedge\psi$),
...
while preserving total mass $O(m)$.  Combined with the flat-norm gluing criterion (Lemma~\ref{lem:prompt8-flat-glue})
...
\begin{hypothesis}[Microstructure/gluing estimate (MG)]\label{hyp:MG}
Let $(X,\omega)$ be a smooth projective K\"ahler manifold of complex dimension $n$,
let $1\le p\le n$, and let $\beta$ be a smooth closed cone-valued $(p,p)$-form
representing a rational Hodge class $[\gamma]$.
Fix an integer $m\ge 1$ (clearing denominators of $[\gamma]$).

\smallskip\noindent
Assume that for every target accuracy $\eta>0$ one can choose a sufficiently fine
cell decomposition (mesh $h\ll 1$) and implement the local holomorphic sheet
construction of Substep~4.1 (using Theorem~\ref{thm:local-sheets} and the
rounding/discrepancy choices of Substep~4.3) so that the resulting raw integral
current
\[
T^{\mathrm{raw}}=\sum_Q S_Q
\]
satisfies the flat-norm boundary smallness
\[
\mathcal F\!\left(\partial T^{\mathrm{raw}}\right)\ \le\ \eta.
\]

\smallskip\noindent
\textbf{Quantitative target for sublinear fillings.}
Write $k:=2n-2p$ for the dimension of the calibrated cycles.
If, in addition, one can arrange along some (necessarily $m$-dependent) parameter
schedule that
\[
\mathcal F\!\left(\partial T^{\mathrm{raw}}\right)\ =\ o\!\left(m^{\frac{k-1}{k}}\right)
\qquad (m\to\infty),
\]
equivalently $\mathcal F(\partial T^{\mathrm{raw}})\le C\,m^{1-\sigma}$ for some
$\sigma>\frac{1}{k}$, then the Federer--Fleming filling constructed from the
flat norm (see Proposition~\ref{prop:MG-filling} below) has mass $o(m)$.
\end{hypothesis}

\begin{proposition}[Flat norm $\Rightarrow$ small-mass gluing correction]\label{prop:MG-filling}
Let $k:=2n-2p$.  Suppose $T^{\mathrm{raw}}$ is an integral $k$-current on $X$ and
let $\eta:=\mathcal F(\partial T^{\mathrm{raw}})$.
Then there exists an integral $k$-current $R_{\mathrm{glue}}$ with
$\partial R_{\mathrm{glue}}=-\partial T^{\mathrm{raw}}$ and
\[
\Mass(R_{\mathrm{glue}})\ \le\ 2\,\eta\ +\ C\,\eta^{\frac{k}{k-1}},
\]
where $C=C(X,k)$ is the Federer--Fleming isoperimetric constant.
In particular, if $\eta\to 0$ then $\Mass(R_{\mathrm{glue}})\to 0$, and if
$\eta=o(m^{(k-1)/k})$ then $\Mass(R_{\mathrm{glue}})=o(m)$.
\end{proposition}

\begin{proof}
By definition of $\mathcal F$, write $\partial T^{\mathrm{raw}}=R+\partial Q$ with
$\Mass(R)+\Mass(Q)\le 2\eta$.  Since $\partial T^{\mathrm{raw}}$ is a boundary, $R$
is null-homologous.  By the Federer--Fleming isoperimetric inequality, there is an
integral $k$-current $Q_R$ with $\partial Q_R=R$ and
$\Mass(Q_R)\le C\,\Mass(R)^{k/(k-1)}\le C\,(2\eta)^{k/(k-1)}$.
Set $R_{\mathrm{glue}}:=-(Q+Q_R)$.
\end{proof}

\begin{remark}[How far the transport/template framework gets toward Hypothesis~\ref{hyp:MG}]
The results of Substep~4.2 prove a substantial portion of the desired gluing estimate,
conditional only on identifying the correct transverse measures and achieving quantitative face matching.
Concretely:
\begin{itemize}
\item Proposition~\ref{prop:transport-flat-glue} reduces $\mathcal F(\partial T^{\mathrm{raw}})$ to
transport bounds $W_1(\mu_{Q\to F},\mu_{Q'\to F})$ across faces, plus a small-angle graph error.
\item Proposition~\ref{prop:template-flat-glue} shows that if the holomorphic sheet families satisfy the
template coherence hypotheses \textnormal{(T1)}--\textnormal{(T2)}, then
$\mathcal F(\partial T^{\mathrm{raw}})\lesssim (h+\varepsilon)\,m$.
\item Combining this with Proposition~\ref{prop:MG-filling}, one sees exactly which \emph{rate} is needed
for sublinear fillings: it suffices to have $h+\varepsilon=o(m^{-1/k})$ (with $k=2n-2p$), equivalently
$\mathcal F(\partial T^{\mathrm{raw}})=o(m^{(k-1)/k})$.
\end{itemize}
What remains open is to verify \textnormal{(T1)}--\textnormal{(T2)} for the holomorphic sheet constructions
and to resolve the degrees-of-freedom tension at fixed $m$ in middle codimension
(Remark~\ref{rem:param-tension}), potentially via the sliver regime
and the weighted bookkeeping estimate of Remark~\ref{rem:sliver-vs-template}.
\end{remark}
...
has boundary supported on the inter-cube faces.  This is the
\emph{microstructure/gluing} step: one needs a quantitative estimate
implying that the boundary mismatch is small in a norm that admits a
small-mass filling; see Remark~\ref{rem:glue-gap}.

\smallskip\noindent
\textbf{Flat-norm-to-filling principle (conditional input).}
Assume that the construction in Substep~4.2 yields a parameter schedule
(mesh $\to 0$ and auxiliary errors $\to 0$) for which
\[
\mathcal F(\partial S)\ =\ \mathcal F(\partial T^{\mathrm{raw}})\ \le\ \eta_\epsilon,
\qquad \eta_\epsilon\xrightarrow[\epsilon\to 0]{}0.
\]
By definition of the flat norm, choose integral currents $R$ and $Q$ with
\[
\partial S \;=\; R + \partial Q,
\qquad
\Mass(R)+\Mass(Q)\ \le\ 2\,\eta_\epsilon.
\]
Since $\partial S$ is a boundary, $R$ is null-homologous.  By the Federer--Fleming
isoperimetric inequality on a compact Riemannian manifold (in dimension
$k:=2n-2p$), there exists an integral $k$-current $Q_R$ with
$\partial Q_R = R$ and
\[
\Mass(Q_R)\ \le\ C\,\Mass(R)^{\frac{k}{k-1}}
\ \le\ C\,(2\eta_\epsilon)^{\frac{k}{k-1}}.
\]
Setting
\[
U_\epsilon \;:=\; Q + Q_R
\]
gives $\partial U_\epsilon=\partial S$ and
\[
\Mass(U_\epsilon)\ \le\ 2\,\eta_\epsilon + C\,(2\eta_\epsilon)^{\frac{k}{k-1}}
...
\]
since $\Mass(U_\epsilon)\to 0$ under the assumed gluing estimate.
...
\begin{theorem}[Automatic SYR for cone-valued forms (conditional on microstructure/gluing)]\label{thm:automatic-syr}
...
\begin{proof}
Assuming Hypothesis~\ref{hyp:MG} (microstructure/gluing; see also
Remark~\ref{rem:glue-gap}) and the conditional statement of
Theorem~\ref{thm:global-cohom}, this is the content of Steps~1--6 above.
...
\begin{theorem}[Effective classes are algebraic (conditional on microstructure/gluing)]\label{thm:effective-algebraic}
...
% ============================================================
\subsection*{Main theorem: Hodge conjecture (conditional on microstructure/gluing)}
% ============================================================
