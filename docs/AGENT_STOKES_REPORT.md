...
## Findings
- **Submanifold integration is now data‑first**: `Hodge/Analytic/Integration/HausdorffMeasure.lean`
  defines `SubmanifoldIntegrationData` (explicit object). The legacy typeclass is now a thin wrapper.
  There is **no concrete data** in the repo yet, so real Stokes/integration remains an assumption.
- **Kähler measure uses explicit data**: `Hodge/Analytic/Integration/VolumeForm.lean`
  defines `KahlerVolumeMeasureData` and builds `VolumeIntegrationData` from it,
  so L2 theory no longer depends directly on submanifold integration.
...
- `SubmanifoldIntegrationData` (HausdorffMeasure.lean): all of `measure2p`, `integral`, and
...
- `VolumeIntegrationData` instance via `kahlerMeasure` depends on `KahlerVolumeMeasureData` (VolumeForm.lean).
...
## Next Steps
- Implement **real** `SubmanifoldIntegrationData` (Hausdorff measure + integration functional).
